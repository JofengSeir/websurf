...
- TS 侧不引 ts-shared：`viewer/src` 无任何 `ts-shared` import（唯一出现是 `viewer/src/core/pose.ts:11` 注释里"与 ts-shared bspYawToCsYaw 一致"的对照说明——该函数仅服务 BSP 出生点路径，与 .replay 无关）。
...
| `src/ts-shared` | **否**：TS 侧零 import | `grep ts-shared viewer/src` → 仅 `core/pose.ts:11` 注释提及对齐（且该式仅 BSP 路径使用） |
...
| `src/serve.py` | **是**（dev 服务器 + dist 内置服务器） | `viewer/package.json:16`、`viewer/scripts/build-dist.mjs:32-95`（打包内嵌 serve.py 文本） |

共享层自身的职责划分见根导航（[../../docs/index.md](../../docs/index.md)）与架构总篇（[../../docs/architecture.md](../../docs/architecture.md)）；websurf-wasm-core 的实现细节见其共享层文档（`docs/wasm-core.md`）。
...
两条线在 **viewer 世界坐标**上会合：BSP 出生的 GLB 与出生点都落在 `map_coords` 的 Y-up 空间（`src/wasm-core/bsp_to_gltf_core/convert.rs:813-816`、`src/wasm-core/model_integrator/mod.rs:1041-1045`），录像帧坐标走同一 `[y,z,x]` 映射（`viewer/src/replay/shavit-replay.ts:481`）——所以 Shavit 帧的绝对世界坐标可直接与场景对齐（HUD 包围盒外提醒 `viewer/src/app.ts:157-188` 只用于暴露映射错误）。
