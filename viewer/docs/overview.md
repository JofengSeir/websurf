...
> 最后核对：2026-08-19。以实际代码为准（`viewer/src/` + `viewer/crates/wasm/` + 共享 `src/wasm-core/`）。
>
...
| 模块 | 说明 |
|---|---|
| 地图导入 | 文件选择 `.bsp` → 主线程 BspProcessor：metadata → spawn → GLB（消费顺序；GLB 导出须最后） |
| 场景构建 | GLB 挂载 + 空间分块合并（数万 primitive Mesh → ~数百块，移植自 test worker-b，已验证）+ 视距/雾按世界包围盒自适应；无 PVS/LOD/mosaic/默认纹理包 |
| 自由视角 | 指针锁定鼠标视角 + WASD 平移 + 空格/Ctrl 升降 + Shift ×4；**人物 = 相机**（pos 脚底 + 眼高 64.09） |
| 位姿通道 | URL 查询参数 / URL hash / `window.viewer` JS API——三通道应用即生效（见 §4） |

## 3. 加载流程（主线程）

```
选择 .bsp → BspProcessor（主线程 wasm 懒初始化）
...
rAF
 ...
 │   right = (cos yaw, 0, −sin yaw)；空格/Ctrl ±Y；Shift ×4）
