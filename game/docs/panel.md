...
**显示状态机**（与 Pointer Lock 耦合，无需额外 ESC 监听）：

```
初始化（未加载地图）→ 面板【必须显示】（加载地图入口）
加载完成 → 面板隐藏，显示「点击画布锁定」
锁定中 ── ESC（浏览器原生退锁）──→ 面板【弹出】（pointerlockchange 事件驱动）
面板打开：点击「关闭并锁定」→ 请求 Pointer Lock → 面板隐藏
任意时刻（含锁定中）按 M / 菜单键 → 手动开关（兜底）
```

实现：`visible = !pointerLocked || !sceneReady`；`pointerlockchange` 驱动显示/隐藏。
...
| 按键 | 键位列表 + 录制（点击 → 按新键 → 保存 localStorage `vbsp:keymap`） |
| 操作 | 灵敏度、Q/E 旋转速度、noclip 速度 |
| 显示 | 准星（显示/颜色/线长/粗细/间隙/描边/中心点）、速度面板模式（横向/横+竖/综合）、**纹理画质**（原始/压缩低清）、近平面参数 |
| 视角 | 自由视角（noclip）切换、移动速度 |
...
- 面板「自由视角」切换 → `set-physics-mode`（双端 `set_noclip`，Rust 侧 noclip_step）。
- noclip 速度：面板可调（200-3000，sprint ×4）。
- 渲染切权威直读（防止双管道撕裂）。
