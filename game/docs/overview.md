...
> 核对基准：2026-09 当前工作区代码（`game/src/`、`game/crates/`、`game/package.json`、共享 `src/ts-shared/`、`src/phys/`）。旧版文档已移入 [archive/](archive/)，本文为全新重写；所有关键论断标注来源代码路径。
> 公共总架构见 [`../../docs/architecture.md`](../../docs/architecture.md)；共享物理细节见 [`../../docs/phys.md`](../../docs/phys.md)、解析层见 [`../../docs/wasm-core.md`](../../docs/wasm-core.md)、通道与校准见 [`../../docs/ts-shared.md`](../../docs/ts-shared.md)。
...
`game/crates/wasm/src/lib.rs` 是薄导出层（约 2300 行）：
- `pub use websurf_phys::phys::PhysWorld;` —— 物理类直接来自共享 crate，game 自己不实现物理（`game/crates/wasm/src/lib.rs:23`）；
...
`game/pkg/websurf_wasm.js` 由 wasm-pack 生成，`game/src/wasm.d.ts` 直接 re-export pkg 类型。契约由 `game/scripts/check-wasm-api.mjs:17-53` 静态校验：
