...
/// 解压输出上限(字节):头部声明的解压长度与流实际膨胀双双封顶,防恶意头/流
/// 触发巨额分配(64 位 OOM abort / wasm32 容量溢出 trap)。真实地图 lump 远低于此值。
const MAX_LZMA_OUTPUT: usize = 1 << 30; // 1 GiB

/// 限制写入量的 [`std::io::Write`] 适配器:超过上限立即报错中断解压。
    ...
    fn write(&mut self, buf: &[u8]) -> std::io::Result<usize> {
        if buf.len() > self.remaining {
            return Err(std::io::Error::new(
                ...
                format!("LZMA 解压输出超过上限 {}", MAX_LZMA_OUTPUT),
            ));
        }
...
pub fn decompress_valve_lzma(data: &[u8]) -> Result<Vec<u8>, BspError> {
    ...
    // 声明的解压长度超上限 → 直接报错,不做预分配
    if uncompressed_len > MAX_LZMA_OUTPUT {
        return Err(BspError::Lzma(format!(
            "解压长度超出上限:{uncompressed_len} > {MAX_LZMA_OUTPUT}"
        )));
    }
    ...
    // 5. lzma-rs 解压(输入 = alone 头 + 流);输出经 LimitedWriter 封顶,防流本身膨胀
    ...
    let mut output = Vec::with_capacity(uncompressed_len);
    {
        let mut limited = LimitedWriter {
            inner: &mut output,
            remaining: MAX_LZMA_OUTPUT,
        };
        lzma_rs::lzma_decompress(&mut std::io::Cursor::new(input), &mut limited)
...
mod tests {
    ...
    fn rejects_oversized_declared_length() {
        // 恶意头声明 4GB 解压长度 → 预分配前直接报错(曾导致 64 位 OOM / wasm32 trap)
        let mut data = Vec::new();
        data.extend_from_slice(b"LZMA");
        data.extend_from_slice(&u32::MAX.to_le_bytes()); // uncompressed_len
        data.extend_from_slice(&0u32.to_le_bytes()); // compressedLength(仅信息)
        data.push(0x5D); // props
        data.extend_from_slice(&(1u32 << 23).to_le_bytes()); // dictionarySize 8MB
