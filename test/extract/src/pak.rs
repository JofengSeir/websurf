...
/// 单个 PAK 条目解压上限(字节):声明尺寸与实际解压双双封顶,防 zip 炸弹
/// (恶意中央目录声明巨大 size → `with_capacity` 大额分配 / read_to_end 无界膨胀)。
pub const MAX_PAK_FILE_BYTES: u64 = 512 * 1024 * 1024;
...
pub fn pak_extract(
    ...
    for i in 0..zip.len() {
        ...
        // zip 炸弹防护:声明尺寸超上限直接拒绝;实际解压经 take 封顶
        if file.size() > MAX_PAK_FILE_BYTES {
            return Err(BspError::Zip(format!(
                "PAK 条目过大:{}(>{MAX_PAK_FILE_BYTES})",
                file.size()
            )));
        }
        let mut buf = Vec::with_capacity(file.size() as usize);
        let mut limited = std::io::Read::take(&mut file, MAX_PAK_FILE_BYTES + 1);
        limited
            .read_to_end(&mut buf)
            .map_err(|e| BspError::Zip(e.to_string()))?;
        if buf.len() as u64 > MAX_PAK_FILE_BYTES {
            return Err(BspError::Zip(format!(
                "PAK 条目解压超过上限 {MAX_PAK_FILE_BYTES} 字节"
            )));
        }
        result = Some(buf);
        break;
...
/// 一个实体:`key -> value` 映射(按 key 字典序存储,BTreeMap 保证;Source 实体 KV 读取不依赖原始顺序)。
