...
pub fn build_glb(primitives: &[PrimitiveData], scene_name: &str) -> Vec<u8> {
    ...
    let json_obj = json!({
        ...
        "nodes": [{
            ...
            // 须与 scene map_coords 的顶点重映射 [x,y,z]→[y,z,x] 组合,整体为 (x,z,-y),两处缺一不可
...
/// 顶点包围盒。空顶点集返回 None(避免产生 min=+inf/max=-inf 的非法 accessor,
/// 非有限值同样返回 None——serde_json 序列化非有限浮点会 panic)。
fn bbox(verts: &[VertexData]) -> Option<([f32; 3], [f32; 3])> {
    if verts.is_empty() {
        return None;
    }
...
mod tests {
    ...
    #[test]
    fn empty_primitive_has_no_min_max() {
        // 空顶点集:accessor 不得输出 min=+inf/max=-inf(违反 glTF 规范且 JSON 序列化 panic)
        let prim = PrimitiveData {
            vertices: vec![],
            indices: vec![],
            material: None,
        };
        let glb = build_glb(&[prim], "empty");
        let json_len = u32::from_le_bytes(glb[12..16].try_into().unwrap()) as usize;
        let json: serde_json::Value = serde_json::from_slice(&glb[20..20 + json_len]).unwrap();
        let acc = &json["accessors"][0];
        assert!(acc.get("min").is_none(), "空顶点集不应有 min: {acc}");
        assert!(acc.get("max").is_none(), "空顶点集不应有 max: {acc}");
    }
