...
pub fn raw_lump<'a>(data: &'a [u8], header: &BspHeader, index: usize) -> Result<Option<&'a [u8]>, BspError> {
    ...
    let lump_err = || BspError::LumpOutOfBounds {
        index,
        offset: entry.offset,
        length: entry.length,
        filesize: data.len(),
    };
    // checked_add:wasm32 上 usize=u32,恶意目录项(offset+length 环绕)会使检查失效
...
mod tests {
    ...
    #[test]
    fn raw_lump_rejects_wrapping_offset_length() {
        // wasm32(usize=u32)上 offset=5 + length=0xFFFFFFFF 会环绕为小值绕过边界检查,
        // 随后 data[start..end](start > end)panic;checked_add 应将其拦截为越界错误
        let mut bytes = make_header(20, 0);
        let lump_off = 8;
        bytes[lump_off..lump_off + 4].copy_from_slice(&5u32.to_le_bytes());
        bytes[lump_off + 4..lump_off + 8].copy_from_slice(&u32::MAX.to_le_bytes());
        let header = BspHeader::parse(&bytes).unwrap();
        assert!(matches!(raw_lump(&bytes, &header, 0), Err(BspError::LumpOutOfBounds { .. })));
    }
}
