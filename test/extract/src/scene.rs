...
pub fn rebuild_scene(bsp: &BspFile) -> Result<Vec<PrimitiveData>, BspError> {
    ...
    let (model_ranges, has_models) = if models.is_empty() {
        ...
        (
            models
                .iter()
                .filter_map(|m| {
                    // first_face/num_faces 为不可信 i32:负值或 i32 加法溢出(debug panic /
                    // release 环绕)都会被拦截,越界 model 直接跳过
                    if m.first_face < 0 || m.num_faces < 0 {
                        return None;
                    }
                    let first = m.first_face as i64;
                    let last = first + m.num_faces as i64;
                    if last > faces.len() as i64 {
                        return None;
                    }
    ...
    for (first, last) in &model_ranges {
        ...
        for face_idx in *first..*last {
            ...
            // 校验顶点链:first_edge/num_edges 为不可信 i32/i16,先验负值再 checked_add
            // (wasm32 上 usize=u32,负值转 usize + 加法环绕会绕过边界检查)
...
fn parse_faces(bsp: &BspFile) -> Result<Vec<Face>, BspError> {
    ...
    // Face v1 = 56 字节;若 size 不符(可能是 v0 28B 或 v2 64B),直接报错拒绝(不做启发式拆分)
