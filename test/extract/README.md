...
> 构建要求:**Rust ≥ 1.87**(代码使用 `usize::is_multiple_of`,另 `div_ceil` 需 ≥1.73)。
...
python serve.py          # http://localhost:8280/test/extract/web/
...
或手动:`python serve.py` 后访问 `http://localhost:8280/test/extract/viewer/`。
查看器功能:
...
- GLB 导出含:世界模型 + brush 实体 + displacement 地形 + UV + 材质名分组 + 根节点 Y+90° 旋转;
  该旋转**必须与 scene 的顶点坐标映射 `[x,y,z] → [y,z,x]`(map_coords)组合**才等于整体 Z-up→Y-up,
  组合结果为 `(x,z,-y)`——两处缺一不可,改动其一须同步另一处;
