...
use world::{add_axial_bevels, Brush, LadderVolume, TriMesh, World};
...
/// 物理事件（一次性消费，供宿主工程统计/记录用；game 未使用不影响）。
#[derive(Clone, Debug)]
pub enum PhysEvent {
    /// 传送触发（目标点信息）。
    Teleport {
...
impl PhysWorld {
    ...
    pub fn build_world(
        ...
        for b in brushes {
            ...
            if b.is_ladder {
                ...
                // P2：brush 凸棱生成轴向 bevel（构建期一次；不扩展 min/max，
                // 空间索引不变；把坡顶入坡幻影碰撞的过近似区切回真实棱）
                let mut brush = Brush {
                    planes,
                    min,
                    max,
                };
                add_axial_bevels(&mut brush);
                self.world.solids.push(brush);
            }
        }
    ...
    fn step_core(&mut self, dt: f64, keys_mask: u32, dx: f64, dy: f64) {
        ...
        if self.noclip {
            ...
            // 落地稳定门槛参数已随 P6 删除——Rust check 不再接收 gate_ticks）
    ...
    /// 取最近一次物理事件（传送/死亡），返回 null 表示无事件；一次性消费。
    /// 返回结构：
    /// - `{ kind: 'teleport', targetname, origin: [x,y,z], yaw }`
    /// - `{ kind: 'death' }`
...
impl PhysWorld {
    fn state_js(&self) -> JsValue {
        ...
        // P3-A1：渲染侧“是否在坡上滑行”（try_player_move 命中 0.05<ny<0.7 面时置位，
        // 每次 tick 末为该 tick 滑行状态）——权威已落地（摩擦域）而渲染线仍在滑行时
        // 跳过速度灌入的精确判定依据。
        set_bool(&obj, "surfing", p.surfing);
