 ...
 * - 权威碰撞事件（land/blocked）postMessage 回传主线程做位置兜底驳回（反向同步权威）
 *
...
export interface AuthLoop {
  ...
  start(): void;
  /** 暂停/恢复自驱链（P4-B1：后台标签页时序黑洞根治）。暂停 = 停止 setTimeout
   * 链 + 清累积器；恢复 = 清基准墙钟 + 重启链（调用方负责先把权威对齐到渲染线
   * 当前状态——见 app.ts visibilitychange）。 */
...
export function createAuthLoop(env: AuthLoopEnv): AuthLoop {
  ...
  let lastWall = 0;
  let started = false;
  /** 暂停标志（P4-B1）：置位则 loop 不续 setTimeout 链，自驱循环自然停摆；
   * 恢复时重排链 + 清累积器（防深度节流欠步狂奔）。 */
  let paused = false;
  ...
  function stepPhysics(dt: number): void {
    ...
    // 权威碰撞事件（低频，postMessage 回传主线程做位置兜底驳回/反向同步）：
  ...
  function loop(): void {
    if (paused) return; // 暂停：不续 setTimeout 链，自驱循环停摆（后台标签页）
    setTimeout(loop, 4);
  ...
  return {
    ...
    setPaused(p: boolean): void {
      if (p === paused) return;
      paused = p;
      // 暂停/恢复都清累积器与基准墙钟：暂停防残留欠步、恢复防深度节流后
      // 的欠步“狂奔”把权威瞬间推到错误位置（与渲染线分叉）
      acc = 0;
      lastWall = 0;
      if (!p && started) {
        loop(); // 恢复：重排链（首个周期只取基准墙钟，不步进）
      }
    },
  };
}
