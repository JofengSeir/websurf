...
export class MsgState {
  ...
  /** P4-B1：后台标签页隐藏时键位清零（消息回退模式：Worker 侧 keysMask 只能经
   * `input` 消息改写——补发一条全零消息；暂停期间到达也不残留按键）。 */
  clearKeys(): void {
    this.keysMask = 0;
    this.post({ type: 'input', dx: 0, dy: 0, keys: 0 });
  }
...
export class ShmState {
  ...
  /** P4-B1：后台标签页隐藏时键位清零（SAB 键位槽置 0 + 清残留鼠标增量；
   * 冻结键位随 rAF 停摆而残留，恢复前必须清零防 Worker 继续移动）。 */
  clearKeys(): void {
    Atomics.store(this.i32, I_KEYS, 0);
    Atomics.store(this.b64, B_DX_ACC, 0n);
    Atomics.store(this.b64, B_DY_ACC, 0n);
  }
