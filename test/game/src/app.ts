...
import {
  createMainSharedState,
  SHARED_BUFFER_SIZE,
  keysToMask,
  KEY_MASK,
  type ShmState,
  type MsgState,
} from './ts-shared/auth/shared-state.js';
import { layerMouseDelta, qeEquivalentDx } from './ts-shared/input/input-layer.js';
import { buildWorldBundle } from './ts-shared/phys/world-builder.js';
...
async function main(): Promise<void> {
  ...
  fixWorker.onmessage = (e: MessageEvent<{ type?: string }>) => {
    ...
    if (msg.type === 'error') {
      ...
      // 权威碰撞事件（落地/撞墙）：位置兜底驳回——不再写回渲染位置，
      // 由 calibrator 反向同步权威到渲染位置（见 AuthorityCalibrator）
  ...
  // P5 修复：场景加载后死亡阈值**双端下发**（渲染 setDeathY + Worker
  // set-death-threshold）——此前仅设渲染侧，权威侧死亡判定用默认 -100000，
  // 坠亡由权威帧兜底掩盖（渲染死亡 → 反向同步拉回）。bridge 在 3 步创建，
  // 但地图加载在用户操作后（bridge 必已就绪）。
...
function bindInput(): void {
  ...
  // P4-B1：后台标签页时序黑洞根治——隐藏时暂停权威自驱链 + 键位清零；
  // 回前台先对齐权威到渲染线当前状态 + 清校准状态，再恢复权威循环。
  // （此前后台 rAF 停摆但 Worker 继续按冻结键位模拟，深度节流后欠步“狂奔”
  //   恢复瞬间与渲染线大幅分叉，被反向同步/撤回反复拉扯）
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      fixWorker?.postMessage({ type: 'pause', paused: true });
      (sharedState as (ShmState | MsgState) & { clearKeys?: () => void })?.clearKeys?.();
      keyboard.reset();
      renderer?.clearPendingInput();
    } else {
      const st = renderer?.getRenderSyncState();
      if (st) {
        // 权威 set_state = 渲染线当前状态（对齐后在恢复的循环里从同一状态继续）
        fixWorker?.postMessage({ type: 'sync-render-state', state: st });
        renderer?.clearCalibrator();
      }
      fixWorker?.postMessage({ type: 'pause', paused: false });
    }
  });
