...
`/test/mini` — 抽取 `/test` 的最核心最小链路：**用户输入 → 物理 → SAB 传输 → 插值渲染输出**。剔除地图导入/导出/BSP/PVS/GLB 渲染，物理用轻量运动学演示，但**架构与完整版一致或更优**。
...
python src/serve.py 8080 test/mini
...
cd test/mini && node scripts/mini-verify.mjs 320 3 64 1200
