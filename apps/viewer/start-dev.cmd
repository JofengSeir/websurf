...
REM ---- web\websurf_viewer_wasm_bg.wasm, the copy npm run build:wasm makes from pkg\. ----
REM Gate: node src/scripts/wasm-stale-check.mjs (pkg\websurf_viewer_wasm_bg.wasm vs newest .rs/.toml
REM under <repo>/src and crates/); exit 0 -> goto :wasm_done, else (or no node) rebuild.
where node >nul 2>nul
if not errorlevel 1 (
  node "%~dp0..\..\src\scripts\wasm-stale-check.mjs" "%~dp0pkg\websurf_viewer_wasm_bg.wasm" "%~dp0..\..\src" "%~dp0crates"
