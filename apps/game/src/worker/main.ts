 ...
 * `rtCur.epoch` = 样本自称的世代；本值 = **服务样本那一刻**复检过的世代
 ...
 *
 * 主线程 `resetTo/换图/noclip/respawn` → `bumpSampleEpoch()` 世代 +1，渲染物理随即
 * 出现在新世界；但 Worker 缓存的那对样本仍属旧世界，而 `rtSampleAtTau` 会把 τ
 * **钳制**到这对样本的 `[rtPrev.t, rtCur.t]` —— 于是一个"看起来合法"的钳制点被
 * 当成当前位置发布（旧世界线上的位置）。复检失败即丢弃配对并在本 tick 返回 null
 * （**绝不跨世代插值**）；调用方 `auth-loop` 回退到权威自身位置发布本帧。
 */
