...
- **框架规范批 2/3/4（尚未落地，不是变更条目）**：本条用于**避免误读**——本 `[修复]` 段与上方「仓库框架规范」段仅记录批 1 的实际改动；批 2（`D-01`/`D-02`/`D-03`：`ensure-node-deps.cmd` 与 `install-wasm-bindgen.cmd` 上提、`check-wasm-api` 引擎抽取）、批 3（端口 10 段槽位、`.cmd` 逐字模板、`start-dev.cmd` 补齐、`web/` 与 `dist/` 形态、许可证唯一源 `D-23`/`E-08`、`T-04` 内核抽取）、批 4（`D-08`/`D-09`/`D-10`/`D-16` 共享层上提）**尚无任何提交可引用**，故此处不写代码范围与验证方式；落地时须逐批补条目并附提交号。记录时点 HEAD = `4523ef1`：`git ls-files src/scripts` 只有 `cargo-env.cmd` 与 `check-doc-drift.mjs`，`src/phys/{LICENSE,NOTICE}` 不存在，`apps/viewer/package.json` 无 `check:api`，三工程 `dev` 仍为 `serve.py 8080`。状态、遗留项与实测判据见 `documents/rollout-status.md`。
