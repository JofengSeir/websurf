...
| 7.2.5 | 光照着色器未上提到共享层（待裁定） | `apps/{game,debug,viewer}` 的 `src/renderer/lightmap-shader.ts`（1667 行）语义相同、字节近乎一致；上提到 `src/ts-shared/render/` 的**唯一阻碍**是 `import * as THREE from 'three'` 的解析：共享层位于仓库根下，而 `node_modules/three` 在各工程内。两条候选：① 仓库根加 `package.json`（npm workspace 或仅 devDependency three）；② 各工程 tsconfig `baseUrl`+`paths` 与 esbuild `--alias:three=<工程>/node_modules/three`。本轮保持工程内副本，未擅自选型 |
