...
> 本轮边界：**只写规范，未搬迁任何代码或文件，未改 `package.json` / `tsconfig.json` / `Cargo.toml` / CI**。§3.3、§6、§7 的文件级动作全部是**待执行**清单，不代表仓库已改。 **【落地状态（收口 `t9` 追加，2026-09-12）】**：`D-01`/`D-02`/`D-03`（批 2 `fc3de84`）、`D-04`/`T-04` 内核与 `D-23`/`E-08` 许可唯一源（批 3 `32c2ddb` + `2135056`）、`D-08`/`D-09`/`D-10`/`D-16`（批 4 `b5be059` ∪ `3b16366`）**均已执行完毕**；`T-05` 门禁 `src/scripts/check-shared-sync.mjs` 并入批 4 落地。**「本轮保留」的 `D-22` 已由第二批（harness 并入规范）部分执行**——`test/dual-mode-harness` 接入 `ensure-node-deps.cmd`/`install-wasm-bindgen.cmd`（引导类）并把端口与输出模板并入规范（[rollout-status.md](rollout-status.md) 的 R-2/R-10/R-11/R-12）；其 `check-wasm-api.mjs` / `build-dist.mjs` 则**按用户裁定保持自带独立实现、不收敛**（验证工程的门禁/构建脚本须与被验对象独立，详见 `D-22`）；除该项外无未决项。逐条处置与遗留登记见 [rollout-status.md](rollout-status.md) §1–§5。
...
- 对象：`apps/debug/`、`apps/game/`、`apps/viewer/` 与共享层 `src/`；`test/dual-mode-harness/` 作为共享工具的**受益方**登记（本文件写作时其文件改造不在 inScope；**第二批已部分并入**——引导脚本与端口/输出模板并入，门禁/构建脚本按裁定保持独立，见 `D-22`）。
...
| D-07 | `Vec3`：`apps/debug/src/physics/math/vec3.ts:11`（vendored）、`apps/game/src/world/types.ts:11,17`（`Vec3Like` + 别名） | 2 处定义（**级别 C**） | **保留** | ①TS 结构化类型下两份 `{x;y;z}` **互相赋值合法**，无编译期耦合（判据：两工程 `npm run typecheck` 均 exit 0）；②debug 那份属 vendored 上游（见 D-06） | 上提单一 `Vec3` → ①需改写 debug 侧 8 处 import（`pvs-manager`、`spawn-loader`、`teleport-manager`、`collider-adapter`、`game-state` 等）；②收益仅 3 行类型定义；③破坏 vendored 边界。【更正】t1 §4.2 记 `Vec3` 有 3 处定义（含 `apps/debug/src/world/types.ts`）；实测该文件不含 `Vec3` 字样（`Select-String 'Vec3' apps/debug/src/world/types.ts` 零命中），是从 `../physics/math/vec3.js` 导入 → 实为 **2 处** |
...
| D-09 | base64 → `Uint8Array` 解码 **8 处 / 4 处落点**——三工程 6 处：`apps/viewer/src/core/bsp.ts:46`、`apps/game/src/renderer/renderer-main.ts:518`、`apps/game/src/world/pvs-manager.ts:273`、`apps/debug/src/main-wasm.ts:30`、`apps/debug/src/default-pack.ts:26`、`apps/debug/src/world/pvs-manager.ts:273`；**共享层 2 处**：`src/ts-shared/auth/worker-dispatch.ts:165`、`src/ts-shared/phys/world-builder.ts:196` | **全仓 TS 侧重复度最高的候选**：8 处 / 4 处落点（三工程 + 共享层） | **上提** → `src/ts-shared/wasm/loader.ts` | ①重复度（8 处同算法；原判 6 处**漏计共享层 2 处**，见 §9.2 更正 7）；②依赖方向：全部只依赖 `atob`/`fetch`/`globalThis`，零工程内部依赖；③变更耦合：配套的「内嵌 or fetch」判定分支写法三工程不同（`if (embedded)` / `if (typeof g.x==='string' && g.x.length>0)`）→ 空串/非字符串注入的容错行为不一致；④是 D-10 的前置（上提后的 `pvs-manager` 必须选一处解码实现） | 不上提、各工程自带 3 行 → ①8 处同算法 + 3 种判定写法属「变更必然同步」；②`globalThis.__VBSP_WASM_B64__` 是构建期写入的全局契约，判定口径不一致会让 single 模式在某些工程静默退回 fetch（file:// 下失败的正是这条路径）。**硬约束**：共享模块**不得 import 任何工程的 `pkg/*`**（**更正**：`apps/debug` 与 `apps/game` 的 pkg 入口**同名** `websurf_wasm.js`，互异只在「应用工程 vs viewer `websurf_viewer_wasm.js` vs harness `websurf_test_wasm.js`」——同名分属两目录更无法静态消歧）→ 只共享「取字节」，`initSync`/`init` 留在工程内。**收敛目标：全仓恰好剩 1 处 `atob`**（= `src/ts-shared/wasm/loader.ts` 自身），判据见 §7.7。反向否决「连 initSync 一起上提」：需在共享层编码三套 pkg 名，违反依赖方向 |
...
| D-16 | `EYE_STAND = 64.09`：Rust 权威 1 处（`src/phys/player.rs:34`）+ **TS 侧 7 处引用位点**——`apps/viewer/src/core/constants.ts:13`、`apps/game/src/renderer/renderer-main.ts:647`、`src/ts-shared/decoupled/decoupled-loop.ts:165`、`src/ts-shared/tick/tick-consumer.ts:183`、`src/ts-shared/auth/shared-state.protocol.test.ts:101`（夹具值）、`src/ts-shared/auth/shared-state.protocol.test.ts:140`（**精确断言** `dstF[8] === 64.09`）、`apps/game/scripts/phys-smoke.mjs:135`（**±0.5 容差断言** `Math.abs(eStand.eyeHeight - 64.09) > 0.5`） | **8 处计数**（1 Rust 定义 + 7 TS 引用）；其中**共享层内部 3 处**；原判 6 处**漏计** `:139` 与 `phys-smoke.mjs`，见 §9.2 更正 8 | **TS 侧上提单点** → `src/ts-shared/phys/constants.ts`；Rust 侧不动 | ①R-18 边界 3：是会变更的**标定常量**；②变更耦合最强：渲染眼高必须等于物理眼高，否则渲染位置整体偏移；③当前 Rust 侧改动无任何检查会失败 | 保持字面量散落 → Rust `EYE_STAND` 改动后 TS 侧 7 处引用静默分叉（其中 **2 处是断言**：`src/ts-shared/auth/shared-state.protocol.test.ts:140` 精确相等、`apps/game/scripts/phys-smoke.mjs:135` ±0.5 容差——断言会「按旧值失败」而不提示常量已变）。**【批 4 落地后的实际状态（本批已将实现由字面量改为符号消费）】该风险已由本批消解**：7 处引用全部改为 import 共享单点、不再复写字面量，两处断言亦改**符号级消费**（`:140` 断言 `dstF[8] === EYE_STAND`；`phys-smoke.mjs:135` 的 `EYE_STAND` 于**运行时从 `src/ts-shared/phys/constants.ts` 解析**、解析失败即 `exit 1`），故断言不再可能「按旧字面量失败」。**同口径漏项已复核**：`64.09` 字面量在 `apps/` 与 `src/`（排除 `node_modules`）已全树扫描，除上列 8 处外**无其他命中**；`shared-state.protocol.test.ts:238,239,282,283,310` 的 `eyeHeight: 64` 是夹具占位值（**非**标定值）不计入，viewer `fly.ts:176,195`、`pose.ts:24` 是**符号级消费**（import 常量）而非字面量复制，同样不计入。**禁止**为「消除重复」把 Rust 常量改成 TS 可注入参数（会改变物理层语义与 wasm 导出面）。跨语言一致性现由**已落地**的 `src/scripts/check-shared-sync.mjs` 子检查 `eye-stand` 保证（实跑通过：Rust `src/phys/player.rs` 与 TS `src/ts-shared/phys/constants.ts` 逐位相等 = 64.09） |
| D-17 | `apps/debug/src/world/types.ts`（198 行）vs `apps/game/src/world/types.ts`（17 行） | **级别 C**：`numstat 6 203` | **保留**（除 D-10 的 PVS 三类） | ①真实差异：debug 有 `WasmTriMesh`/`WasmBrush`/`WasmSpawnReport`/`WasmTeleport*`/`WasmBspMetadata`/`ColliderFilter`，game 只余 PVS 类型；②新管线的世界契约已由 `src/ts-shared/phys/world-builder.ts` 承载（`BspProcessorLike`/`WorldMetadata`/`WorldBundle`） | 把 `types.ts` 整体上提为共享「WASM JSON 契约」→ 与 `world-builder.ts` 已消费的契约形成**两套并存契约类型**（正是 D-18/§8.3 要避免的形状） |
...
| D-22 | `test/dual-mode-harness/scripts/*.mjs`（12 个） | — | **保留**（本轮）→ **第二批部分并入**（政策变更，2026-09-12） | 验证工程不参与部署（`.github/workflows/deploy-pages.yml` 头注）；其 `check-wasm-api.mjs` 与 `build-dist.mjs` 曾被列为 D-03/D-04 的**受益方** | 本轮不改造：`test/` 不在本任务 inScope，登记为共享工具上线后的第二轮接入对象（§6.2 T-03/T-04 的「可选消费方」）。**第二批执行结果（更正）**：接入 `ensure-node-deps.cmd`/`install-wasm-bindgen.cmd`（**引导类**）、端口改 `8110`、输出模板并入 §2.5；但 `check-wasm-api.mjs` 与 `build-dist.mjs` **不收敛**——二者是**门禁/构建脚本**，必须与被验对象独立，故保持自带实现。原「收敛为薄配置/薄入口」的裁定由用户在第二批执行中**否决**，理由：验证工程若依赖被验对象，共享层出缺陷时两边一起错，验证失效 |
...
| D-03 | `src/scripts/lib/wasm-api-contract.mjs` | **三份** `scripts/check-wasm-api.mjs` 收敛为薄配置（viewer 批 2 新建；**harness 除外**，按 `D-22` 保持自带实现） | `package.json` 的 `check:api` 保持指向工程内脚本（路径不变，CI 无需改） |
| D-04 | `src/scripts/lib/dist-pack.mjs` | **三份** `build-dist.mjs` 收敛为薄入口（**harness 除外**，按 `D-22` 保持自带实现） | 与 t2 §10.1 第 10/11/12 条合并执行（§7.5 排序约束） |
...
| D-16 | `src/ts-shared/phys/constants.ts` | TS 侧 **7 处**引用（含 1 处精确断言 `shared-state.protocol.test.ts:140` + 1 处 ±0.5 容差断言 `apps/game/scripts/phys-smoke.mjs:135`） | Rust `src/phys/player.rs:34` 不动 |
...
- 保留（不上提）：D-05、D-06、D-07、D-12、D-14、D-15、D-17、D-19、D-20、D-21；D-22 由第二批**部分并入**——引导/环境类共享脚本（`cargo-env`/`ensure-node-deps`/`install-wasm-bindgen`）与端口、输出模板已并入规范；**门禁/构建脚本（`check-wasm-api.mjs`、`build-dist.mjs`）保持自有独立实现**（验证工程不得依赖被验对象），登记见规范 §3.2 豁免表。
- 保留 + 门禁：D-11。
...
| T-03 | `src/scripts/lib/wasm-api-contract.mjs` | 新建引擎（D-03） | 三工程（harness **不接入**，按 `D-22` 保持自带实现） | 批 2 |
| T-04 | `src/scripts/lib/dist-pack.mjs` | 新建内核（D-04） | 三工程（harness **不接入**，按 `D-22` 保持第 4 份自带实现） | 批 3 |
...
- 被谁调用：`apps/<app>/scripts/check-wasm-api.mjs`（薄配置，各约 30–100 行：声明本工程 pkg 名与 API 表 → 调用引擎 → 打印并 `process.exit`）。`test/dual-mode-harness/scripts/check-wasm-api.mjs` **不调用本引擎**（按 `D-22` 保持自带实现 56 行）。`package.json` 的 `check:api` **路径不变**，因此 **CI 无需改动**。
...
| 工程 `scripts/` 下的校验薄配置 | `apps/<app>/scripts/check-wasm-api.mjs` | `D=3` | `../../../src/scripts/lib/wasm-api-contract.mjs`（**harness 同深度但不引**，按 `D-22` 自带实现） |
| 工程 `scripts/` 下的构建薄入口 | `apps/<app>/scripts/build-dist.mjs` | `D=3` | `../../../src/scripts/lib/dist-pack.mjs`（**harness 同深度但不引**，按 `D-22` 自带实现） |
| harness 入口（**第二批已接入引导类**） | `test/dual-mode-harness/play.cmd` | `D=2` | `..\..\src\scripts\ensure-node-deps.cmd`（与 `apps/<app>/` **同为 `D=2`**；「harness 多一层 `test/`」的说法不成立，实测 `%~dp0` 引用 29/29 可达） |
| harness 脚本（**不接入共享层**） | `test/dual-mode-harness/scripts/check-wasm-api.mjs`、`scripts/build-dist.mjs` | `D=3` | **不写**（自有独立实现，见 `D-22`；判据 `git grep -n "src/scripts" -- test/` 零命中） |
| CI workflow | `.github/workflows/deploy-pages.yml` | `D=0` | **禁止**手写深度：用 `working-directory` |

- **【禁止】**用绝对路径或 `cd` 跳出工程目录后再引用（t2 §3.3 同款条文）。
- 层数反例（**历史**：批 1 `fa5552e` 已就地修层、批 2 `fc3de84` 已把该文件上提到 `src/scripts/`，现为同目录 `%~dp0cargo-env.cmd`）：`apps/debug/scripts/install-wasm-bindgen.cmd:19` 少一层 → 解析为 `apps/src/scripts/cargo-env.cmd`（不存在）。本提案通过「共享脚本调用共享脚本写同目录」消除这一类错误面。
...
3. 新建 `src/ts-shared/phys/constants.ts`（导出 `EYE_STAND = 64.09`）；TS 侧 **7 处**改 import（含 2 处断言：`src/ts-shared/auth/shared-state.protocol.test.ts:140`、`apps/game/scripts/phys-smoke.mjs:135`）。
...
| `check-wasm-api.mjs` 收敛为薄配置 | `documents/architecture.md` 明文断言「`check-wasm-api.mjs` 存在于 **debug / game / harness 三处**」 | 该断言在批 2 后**失真**（三处仍在，但实现变为薄配置 + 共享引擎）；收口轮 R-5 已按实测改写为「四工程各一份薄配置 + 共享引擎」；**第二批按用户裁定反转**——harness 那份**不收敛**，保持自有独立实现（`D-22`），`documents/architecture.md:105` 随之改为「三工程薄配置 + 共享引擎，harness 为自带独立实现」 |
...
| # | 对象 | 上游说法 | 实测更正 / 补充 | 证据 |
|---|---|---|---|---|
| 1 | [framework-audit.md](framework-audit.md) §4.2 | `Vec3` 在仓库有 **3 处**定义（含 `apps/debug/src/world/types.ts`） | 实为 **2 处**：`apps/debug/src/physics/math/vec3.ts:11`（vendored）与 `apps/game/src/world/types.ts:11,17`（`Vec3Like` + 别名）；debug 的 `world/types.ts` 不含 `Vec3` 字样 | `Select-String 'Vec3' apps/debug/src/world/types.ts` 零命中；该文件的 `Vec3` 来自 `../physics/math/vec3.js` |
...
| 7 | 本文件 D-09（原判） | base64 → `Uint8Array` 解码「6 处 / 3 工程」（**漏计共享层内 2 处**） | **更正为 8 处 / 4 处落点**：补 `src/ts-shared/auth/worker-dispatch.ts:165`、`src/ts-shared/phys/world-builder.ts:196`；判据同步改为全子树匹配（§7.7 批 1 验证行由 `-- apps/*/src` 改为 `-- apps src`，期望值由「0 处」改为「恰好 1 处 = `src/ts-shared/wasm/loader.ts`」） | 全树 `atob` 扫描实测 8 处（三工程 6 + 共享层 2） |
| 8 | 本文件 D-16（原判） | `EYE_STAND` 同值「6 处」（**漏计 2 处 TS 引用**） | **更正为 8 处计数 = 1 Rust 定义 + 7 TS 引用**：补 `src/ts-shared/auth/shared-state.protocol.test.ts:140`（精确断言 `dstF[8] === 64.09`）与 `apps/game/scripts/phys-smoke.mjs:135`（±0.5 容差断言 `Math.abs(eStand.eyeHeight - 64.09) > 0.5`）。**同口径漏项复核结论：无其他命中**——`64.09` 在 `apps/`+`src/`（排除 `node_modules`）全树扫描仅此 8 处；`:237,238,281,282,309` 的 `eyeHeight: 64` 属夹具占位值、viewer `fly.ts:176,195` 与 `pose.ts:24` 属符号级消费，均**不**计入同口径 | 字面量全树扫描；`apps/game/scripts/phys-smoke.mjs:118-123` 第 7 项断言 |
