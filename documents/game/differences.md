...
| game 特有 | 证据 |
|---|---|
| `PanelController` 独立类（七模块 + 偏好持久化 + 存点列表渲染） | `apps/game/src/panel/panel-controller.ts`（808 行）；debug 无 panel/ 目录（`ls apps/debug/src` 无 panel/ui，面板逻辑在 `apps/debug/src/app.ts` 直绑 HTML） |
