...
window.mousemove（仅锁定时）
  → MouseBuffer.process(movementX, movementY)          apps/game/src/input/mouse-buffer.ts:56
...
### 2.3 按键门控（`apps/game/src/input/keyboard.ts:41-115`）
...
## 3. PointerLock（`apps/game/src/input/pointer-lock.ts`）
