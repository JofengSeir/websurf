...
> 核对基准：当前 `apps/game/src/savepoint.ts`、`apps/game/src/app.ts`、`apps/game/src/renderer/renderer-main.ts`、`apps/game/src/world/*`。总览见 [../overview.md](../overview.md)，输入/面板见 [panel-and-input.md](panel-and-input.md)。

## 1. 存点系统（X 存点 / C 按住冻结）

### 1.1 存储（`apps/game/src/savepoint.ts`）
...
### 1.2 X 键存点（`apps/game/src/app.ts:205-213,482-489`）

仅锁定时响应：`renderer.getFullState()`（读主线程物理实例完整状态，`renderer-main.ts:551-573`）→ `savePointStore.add({...s, t})` → 面板列表刷新。

### 1.3 C 键按住 = 定在最近存点（`apps/game/src/app.ts:210-220,491-510`）
...
- 共享 mosaic/MTZ 技术详见 [`../../wasm-core.md`](../../wasm-core.md) §3.7（根解析层文档，系统承载 mosaic 字节码与 MTZ 容器；differences.md §4 同一指法）。
...
grep `apps/game/src` 全量检索 `game-state|计时|challenge|checkpoint|timer`：仅命中 `config.lockTickRate` 的注释（"计时玩法公平性"预留开关，`apps/game/src/config.ts:81-93`）与面板注释（`panel-controller.ts:222`）——**game 当前没有计时挑战/检查点系统**；`worker-dispatch.ts:151-155` respawn 注释明示"计时挑战检查点回退已移主线程"是 debug 侧设计。debug 的 `game-state.ts` 计时系统未移植到 game（对照证据见 [../differences.md](../differences.md) §2）。
