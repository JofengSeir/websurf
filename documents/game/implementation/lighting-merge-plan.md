...
#### 10.6.4 移动中的帧时间实测（含**阴性结果**，2026-09-21）

方法论：同一张图（surf_666）、同一按键与鼠标节奏（按住 W + 每 120 ms 小幅转视角，保证新几何持续入视锥），
交替顺序 `baked → texture → baked → texture` 各 8 s，页内 rAF 采样帧间隔；每档跑 2 段取均值。

| 配置 | 模式 | FPS | median | p95 | p99 | 最长帧 | 长帧(>3×中位) |
|---|---|---|---|---|---|---|---|
| game 1920×1080 | baked | 323.0 | 3.11 | 4.44 | 4.87 | 7.91 | 0 |
| game 1920×1080 | texture | 323.5 | 3.11 | 4.45 | 4.92 | 7.34 | 0 |
| game 3840×2160 | baked | 320.1 | 3.13 | 4.45 | 4.88 | 6.77 | 0 |
| game 3840×2160 | texture | 323.9 | 3.09 | 4.46 | 4.94 | 9.37 | 1 |
| debug 1920×1080 | baked | 323.1 | 3.13 | 4.43 | 4.84 | 6.44 | 0 |
| debug 1920×1080 | texture | 320.4 | 3.13 | 4.43 | 4.94 | 26.87 | 4 |

**结论（如实记录）**：在**本测试环境**（headless Edge，`--headless=new`）两模式移动中的帧时间**无差异**，
且 1080p 与 4K 都稳定在 ~320~324 FPS（≈3.1 ms/帧）⇒ 帧节奏被限幅、根本没进入片元受限区，
**这台机器测不出光照项开销差**。因此：

- 可证的只有两点：① 纯纹理的**每帧工作量严格更小**（少一次图集采样 + 解码 + 逐顶点/环境盒加权，见 §10.6.2 的分支）；
  ② **切换本身不产生跳变**（0.2~0.6 ms、无 >100 ms 帧、分块与 draw 数与模式无关）。
- 面板小字因此只写**开销方向**（"每帧光照开销更大 / 最小"）与用途（"移动时渲染速度不要大幅跳变"），
  **不写**"帧时间更容易跳变"这类量化承诺——那需要一台片元受限（弱 GPU / 更高分辨率 / 更重地图）的机器复测。
- 复现脚本为一次性探针（`.tmp/cdp-motion-timing.mjs`，按 AGENTS §3 不入库）；产物在 `temp/shots/motion-*-result.json`。
