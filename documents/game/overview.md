...
> 核对基准：2026-09 当前工作区代码（`apps/game/src/`、`apps/game/crates/`、`apps/game/package.json`、共享 `src/ts-shared/`、`src/phys/`）。旧版文档已移入 [archive/](archive/)，本文为全新重写；所有关键论断标注来源代码路径。
...
| `apps/game/src/renderer/renderer-main.ts` | 1024 | 渲染主线：Three.js 初始化、GLB 场景挂载、分块合并 optimizeScene、LOD/PVS、近平面自适应、主线程物理 tick、权威校准入口、画质切换 |
| `apps/game/src/worker/main.ts` | 93 | Worker 装配：`createAuthLoop` + `createWorkerDispatch`，`getConfigTickRate = config.physics.tickRate + TICK_RATE_OFFSET`（`:86`） |
...
| `apps/game/src/input/mouse-buffer.ts` | 128 | `process()` 路径：discardNext + 单事件削平 ±1000（`MAX_DELTA` `:40`；`push/drain` 为遗留未用路径） |
| `apps/game/src/input/pointer-lock.ts` | 154 | `unadjustedMovement:true` 请求 + 旧浏览器 void 降级 + 3s 超时（`:71`） |
...
- 一键：`apps/game/play.cmd:32-62` 四步自举（ensure-node-deps → wasm → ts → dist）后以 `game/serve.py` 起服务（**COOP/COEP + no-store**，`game/serve.py:27-34`，SAB 生效前提）自动打开 `http://localhost:8137/dist/index.html`。
- dev 页面：`python src/serve.py 8080` 后访问 `/game/web/index.html`（需先 `npm run build:ts`）。
- ⚠️ 仓库内已有 `apps/game/web/*.js` 与 `game/dist/*` 可能是旧架构（v3）产物——运行前先重建（`apps/game/README.md` 已明示）。

## 6. WASM 契约（双端共用一个包）

`game/pkg/websurf_wasm.js` 由 wasm-pack 生成，`apps/game/src/wasm.d.ts` 直接 re-export pkg 类型。契约由 `apps/game/scripts/check-wasm-api.mjs:17-53` 静态校验：
...
> 归档旧文档（v5 时代视角，仅供历史对照）：[archive/](archive/)。
