...
## 1. 参数定义（`apps/debug/src/physics/param-defs.ts`）
...
## 3. Worker 协调器（`apps/debug/src/worker/physics-worker.ts`，114 行）
...
## 5. UI 组织（`apps/debug/web/index.html` + `app.ts` bindUI）
...
- worker-dispatch 的五个 debug 注入点中，面板直接占三个：`onWorldBuilt`（attachWorld）、`onConfigApplied`（reapplyParams）、`onExtraMessage`（5 种消息）；`onInit`（ready 回执）与 `onWasmInit`（mtz 存取，协议兼容保留）与面板无关（`apps/debug/src/worker/main.ts:106-119`）。
- `set_params`/`set_hull` 的完整字段语义在共享层：[documents/phys.md](../../phys.md)（PhysWorld 21 导出、PhysParams 默认表）。
