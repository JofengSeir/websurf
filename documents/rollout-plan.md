...
> 范围裁定（captain，2026-09-12）：端口/输出的**判据范围 = `apps/`**；`test/dual-mode-harness/` 本轮显式排除（D-22、§9.1-G-3），其 8080 → 8110 的端口差异登记为遗留项（§10 第 3 条）。**（补注（第二批）：本裁定的「判据范围 = `apps/`」是**批 3 当期**的范围，不代表现行状态——harness 已并入规范，端口判据现覆盖 `apps` + `test/dual-mode-harness`，见 [rollout-status.md](rollout-status.md) §5。）****`T-04` 内核抽取在本批内新建并当批消费**（§9.1-G-1/G-2）。
...
| B3-1 | 端口重编号为 10 段固定槽位（debug 8080/8081 不变；**判据 = `apps/` 下的「可执行端口面」**） | 改文本：`apps/game/package.json:15`（`dev` 8080→8090）、`apps/viewer/package.json:17`（→8100）、`apps/game/play.cmd:6`（8137→8091）、`apps/game/build-dist.cmd` 原 `:97-98`（文案 8137→8091；**批 3（`32c2ddb`）模板重写后该文案已整段移除**——现文件仅 93 行且不再打印端口，故此锚点为**历史**，端口文案只余 `play.cmd`）、`apps/viewer/play.cmd:12,15`（8090→8101）、`apps/viewer/scripts/build-dist.mjs:104,108`（内嵌 `dist/play.cmd` 模板 8090→8101） | 可执行面之外**无同步项**：README 与 `documents/` 的文字同步**显式排除本批**，统一移入收口项 §7.4 S-1（G-6 裁定） | **【裁定（captain，2026-09-12）：可执行面 = `.cmd` / `package.json` / CI】** ① `git grep -nE "set PORT=[0-9]+" -- apps` 取值集合 = {8080, 8081, 8091} ∪ {8101}（8101 来自 `apps/viewer/scripts/build-dist.mjs:108` 的内嵌模板，必须一并收敛否则判据①永假）；② `git grep -nE "serve\.py [0-9]+" -- apps` ∈ 表；③ `localhost:<表内端口>` 无表外；④ `git grep -n "8137" -- apps .github` **空**；⑤ **显式排除**：`test/dual-mode-harness/`（G-3 裁定）与全部 `README`/`documents/` 文字（G-6 裁定 → §7.4 S-1）——本判据**不代表**「全仓一切一致」。**补注（第二批）**：harness 已并入规范（端口 `8110`、共享工具接入），`git grep -nE "set PORT=[0-9]+" -- apps test/dual-mode-harness` 现取值 {8080, 8081, 8090, 8091, 8100, 8101, 8110}；G-6 的 README/`documents/` 文字同步**仍有遗留**（见 S-1 行的补注） | R-02、R-03、R-05、§2.3、§10.1 第 3/4/5 条；§9.1-G-3 / G-6 裁定 |
...
| # | 收口项 | 具体对象 | 判据 |
|---|---|---|---|
| S-1 | **端口文档同步**（G-6 裁定：文档面收口） | 根 `README.md:32`、`documents/debug/README.md:21-22,31`、`documents/game/README.md:58`、`documents/viewer/README.md:30,202`、[documents/game/overview.md](game/overview.md):107、`documents/dual-mode-harness/README.md`、`documents/dual-mode-harness/overview.md:100` | `git grep -nE "\b(8080\|8081\|8090\|8091\|8100\|8101\|8110\|8137)\b" -- README.md apps documents test` 的每条命中都等于规范 §2.3 表值；`8137` **零命中**（harness 文件的 8080→8110 随 harness 另案，见 §10 第 3 条）。**补注（第二批）**：harness 两处已同步（`documents/dual-mode-harness/README.md`、`documents/dual-mode-harness/overview.md:100` 端口均改 `8110`）；**apps 侧与 `documents/{game,viewer}` 的端口文案仍为旧值**（`documents/game/README.md:58`、`documents/viewer/README.md:30,202`、`documents/game/overview.md:108`、`documents/viewer/overview.md:39`），不属第二批 inScope，如实登记为未闭合项 |
| S-2 | `architecture.md` 的 `check-wasm-api` 三处断言（G-7 裁定） | [documents/architecture.md](architecture.md):105 | 断言与批 2 后的实际形态一致（「薄配置 + 共享引擎 `src/scripts/lib/wasm-api-contract.mjs`」，**不是**「三处各自实现」）。**补注（第二批）**：harness 那份**不并入**该形态（自有独立实现，见 `D-22`），`architecture.md:105` 已随之改为「三工程薄配置 + 共享引擎；harness 自带独立实现」 |
...
| 6 | `CHANGELOG.md:92`（2026-09-13 按**内容**重定位，原 `:86` 行内无该路径） | 同上 |
...
3. **不动 `test/dual-mode-harness/`**：D-22 判「保留（本轮）」；其 `check-wasm-api.mjs` 与 `build-dist.mjs` 是共享工具的**第二轮**可选消费方。**G-3 已裁定：harness 端口本轮显式排除**（不扩 t4 `inScope`）——现状 `set PORT=8080`（`test/dual-mode-harness/play.cmd:6`）与 `dev`（`test/dual-mode-harness/package.json:13`）对照规范目标 8110 的差异，连同 `documents/dual-mode-harness/README.md`、`documents/dual-mode-harness/overview.md:100` 的端口文案，**统一登记为 harness 另案的遗留项**。**补注（第二批）**：本项已由「harness 并入规范」执行（`D-22` 政策变更）——端口改 `8110`、`check-wasm-api.mjs`/`build-dist.mjs` **按用户裁定保持自有独立实现**（不收敛，见 `framework-decoupling.md` 的 `D-22`）、`[IMPORTANT]` 归一为 `[INFO]`，故本节第 3 条不再成立；第 1/2/4–11 条继续有效（见 [rollout-status.md](rollout-status.md) §5 的 R-2/R-10/R-11/R-12）。
