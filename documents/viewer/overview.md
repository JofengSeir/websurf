...
| 环节 | 命令 | 产物 | 来源 |
|---|---|---|---|
| WASM 构建 | `npm run build:wasm` | `apps/viewer/pkg/websurf_viewer_wasm.*`（wasm-pack release、target web），并把 `*_bg.wasm` 拷到 `web/` | `apps/viewer/package.json:8` |
...
| 单文件打包 | `npm run build:dist` | `apps/viewer/dist/`：IIFE `app.js` 内嵌 WASM(base64)+Worker 代码、classic `index.html`、`assets/maps/surf_null_4.replay` 示例、serve.py、play.cmd/play.sh | `apps/viewer/package.json:15`、`apps/viewer/scripts/build-dist.mjs:1-16` |
