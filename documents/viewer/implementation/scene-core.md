...
## 1. ViewerScene（`apps/viewer/src/core/scene.ts`，498 行）

**职责边界**（文件头注释 `scene.ts:1-5`）：renderer/scene/camera/灯光/GLB 挂载/**静态光照（预烘焙）**/空间分块合并/相机 near-far 自适应；**不持有输入与位姿状态**（那些在 `fly.ts`），只提供挂载与渲染能力。渲染方法与 game `renderer-main` 对齐：三点光、近平面贴墙自适应、无雾、`far = maxDim × 100`（`scene.ts:2`、`constants.ts:14-15`）。

### 1.1 构造（`scene.ts:72-112`）

- `WebGLRenderer({canvas, antialias, powerPreference:'high-performance'})`，`setPixelRatio(min(devicePixelRatio, 2))`、`outputColorSpace = SRGBColorSpace`（`scene.ts:73-80`）。
- **静态光照参数（2026-09-21 接入）**：`setExposure(2.3)` / `setLightGamma(2.2)` / `setAmbientScale(1)` / `setPropVertexRelax(1)` / `setPropVertexFlatten(0.85)` / `setLightingModeInShader('baked')`（`scene.ts:82-92`）——注释明示**逐字对齐 `apps/game/src/config.ts:238-247`** 的 `DEFAULT_CONFIG.lighting`；首版误填「外部参照实现平价」1 / 0.5 会把画面整体压暗（γ 0.5 ⇒ 指数 2 = 平方衰减）。
- 背景色 `BG_COLOR = 0x0d1b2a`（`scene.ts:103`、`constants.ts:22`）。
- 相机 `FOV=73.6`、初始 near/far `0.1 / 100000`（`scene.ts:105-111`、`constants.ts:9-13`）。
- 三点光：Ambient 0.6 + Hemisphere(0xb0c4de/0x404030, 0.4) + Directional(0xfff4e0, 0.5)（`scene.ts:107-112`，注释"与 game renderer-main 相同的三点光组合"）。世界材质经静态光照后是 `MeshBasicMaterial` ⇒ 不受这三点光影响，它们只作用于未收敛的辅助对象。

### 1.2 GLB 挂载（`scene.ts:152-189`）

`mountGlb(glbBytes)`：字节拷贝 → Blob URL → `GLTFLoader.loadAsync` → revoke；旧地图 `disposeObject` + remove（防重复加载泄漏，`scene.ts:164-168`）；`resetRootRotations` 清 GLB 根节点旋转（`scene.ts:490-497`，注释"与 game renderer-main resetRootRotations 同法"）；**`applyStaticLighting`（必须先于分块合并）** → `optimizeScene()` → `fitCamera()`。`disposeObject` 递归 dispose geometry/texture/**lightMap atlas**/material（`scene.ts:469-488`；atlas 释放的理由：viewer 是反复换图的工具，漏掉会每张图多留一张 4096×2048 图集）。

### 1.2.1 静态光照与光照模式开关（2026-09-21 新增）

- `applyStaticLighting(gltf, root)`（`scene.ts:192-217`）：`loadLightmapAtlas(gltf.parser, gltf)`（atlas 取自 `asset.extras.lightmap.textureIndex`）→ `applyLightmapToMeshes(root, atlas)` → 终扫 `fullbrightUnlitLitMaterials(root)`（仍为受光材质的图元收敛为贴图原色）；失败只告警不阻断。
- **必须在 `optimizeScene` 之前**：分块合并按材质实例分组，合并后再换材质会让合并失效，且逐 primitive 的 `hasLightmap` / `TEXCOORD_1` 映射会被丢掉。
- `setLightingMode(mode)` / `getLightingMode()`（`scene.ts:219-230`）：与 apps/game、apps/debug **同一语义的运行期性能旋钮**（只改共享 uniform `vbspBakedMix`，不重建场景、不重编译材质）。面板入口见 §6.3。
- 着色器实现为工程内副本 `apps/viewer/src/renderer/lightmap-shader.ts`（与 game/debug 两份**逐字节同源**，三份 sha256 一致），上提共享层的阻碍见 `AGENTS.md` §7.2.5。

### 1.3 相机 near/far 自适应（`fitCamera`，`scene.ts:235-252`）

地图加载后：`near = max(maxDim/1000, 0.05)`（`CAMERA_NEAR_MIN`）、`far = max(maxDim × 100, 初始 far)`——注释明示"原 diag×2 + 65536 截断会裁掉超大地图远景"（`scene.ts:236-247`）。`CAMERA_FAR_SCALE=100` 标注"与 game loadScene 一致"（`constants.ts:14-15`）。

### 1.4 近平面贴墙自适应（`updateNearPlane`，`scene.ts:254-317`）

每 2 帧执行一次（`render()` 里的 `nearCheckToggle`，`scene.ts:134-141`）：

1. **包围球粗筛**（BSP 模型子树内 `modelRoot.traverse`，几何包围球中心距相机 `< probe×2 + radius` 才入选，`scene.ts:261-275`）；
2. **6 方向探测**：相机局部系前/后/左/右/**上/下**（`scene.ts:283-297`）——注释明示与 game 的差异："game 为 4 水平方向，查看器自由飞行会贴地/贴顶，补垂直两向"（`scene.ts:255-257`；game 侧实况：`apps/game/src/renderer/renderer-main.ts:381-424` 的 `dirs` 为 4 个水平正交方向）；
3. 探测命中 → `near = max(minDist × nearRatio(0.3), 0.05)`；空旷 → 恢复 `defaultNear`（`scene.ts:303-316`，常量 `constants.ts:16-21`：`CAMERA_NEAR_MIN=0.05`、`NEAR_PROBE_DIST=100`、`NEAR_RATIO=0.3`，均标注与 game 默认一致）。

### 1.5 空间分块合并（`optimizeScene`，`scene.ts:319-467`）

- 来源：从 test/dual-mode-harness 移植的合并算法（`scene.ts:319-329` 注释"移植自 test/dual-mode-harness worker-b.ts"）。
- 目标块数与 cell 尺寸：目标 512 块，下限 300 / 上限 800，cell 在 128–4096 HU 间迭代（最多 6 次）求满足目标的最粗 cell（`scene.ts:36-44` 常量、`scene.ts:370-387` 迭代）。
- 空间分桶：`optCellKey = floor(x/cell)|floor(y/cell)|floor(z/cell)`（`scene.ts:45-47`）。
- 同 cell 内**按材质分组**再 `mergeGeometries`（`scene.ts:396-436`）；多材质 Mesh（material 数组）不进分桶——世界变换烘焙进几何后单独保留（`keptMeshes` 路径，`scene.ts:322-338`）。
- **失败兜底**：最终合并失败（indexed/non-indexed 属性不兼容）时**全部单独保留**而不是只留第一块——注释"与 game 同法——只留第一块会把该 cell 其余几何静默丢掉（渲染不全根因）"（`scene.ts:442-451`）。
- **视锥外保一圈**：合并后重算包围球并 `radius ×= FRUSTUM_PAD(1.6)`（"快移/猛转时新入视锥几何已预渲染"，`scene.ts:42-43, 454-462`）。
- 旧根移除、新根挂回 `modelRoot`（`scene.ts:464-467`）。

### 1.6 拾取接口

`hasModel()` / `model` getter（拾取用）/ `worldBox()`（地图贴合检查/录像对照用，`scene.ts:113-127`）。
...
## 6. MapPanel（`apps/viewer/src/ui/mapinfo.ts`，208 行；ReferenceGrid 已移除）

### 6.1 MapPanel（地图页全部内容）

- 「更换地图」入口：加载成功后显示，点击走 `#bspFile.click()` 同一链路（`mapinfo.ts:45-55`；加载中 `setLoadBusy` 禁用，`mapinfo.ts:70-72`）。
- **「光照模式」区块（2026-09-21 新增，`mapinfo.ts:79-115`）**：`section(root, '光照模式')` + `<select id="lightingMode">`（预烘焙 default / 纯纹理）+ 同款性能小字；`change` → 构造参数 `onLightingMode` 回调 → `app.ts` 里 `scene?.setLightingMode(mode)`。语义与 apps/game、apps/debug 完全一致（**移动时的渲染速度**旋钮，运行期 uniform 切换、不重建场景），默认**预烘焙**（用户 2026-09-21 定调）。
- 地图信息：默认核心三行（文件 / 出生点数 / 世界尺寸 X×Y×Z HU，`mapinfo.ts:130-145`）；magic/brushes/faces/models/vertices/static props/PAKFILE 数/解析耗时/包围盒收进「统计明细」折叠（`mapinfo.ts:147-160`）。
