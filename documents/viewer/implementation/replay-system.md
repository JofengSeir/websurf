...
## 1. 数据契约层（`apps/viewer/src/replay/types.ts`，164 行）
...
## 2. 原生 .replay 解析器（`apps/viewer/src/replay/shavit-replay.ts`，584 行）
...
### 4.1 消息协议（`apps/viewer/src/replay/protocol.ts`，35 行）
...
### 4.2 解析 Worker（`apps/viewer/src/worker/main.ts`，110 行）
...
### 4.3 导入器（`apps/viewer/src/replay/importer.ts`，176 行）
...
### 5.1 轨道集合（`apps/viewer/src/replay/tracks.ts`，110 行）
...
### 5.2 播放器（`apps/viewer/src/replay/player.ts`，206 行）
...
### 5.3 采样纯函数（`apps/viewer/src/replay/sampling.ts`，80 行）
...
## 6. 3D 可视化（`apps/viewer/src/replay/visuals.ts`，174 行）
...
### 7.1 录像面板（`apps/viewer/src/replay/panel.ts`，318 行）
...
### 7.2 轨迹面板（`apps/viewer/src/replay/trackpanel.ts`，214 行）

每轨一行（显隐/配色/名称/时间偏移/跟随/移除）+ 批量操作（全部显示/全部隐藏/偏移归零/清空全部，
仅在有轨道时出现，`:47-79`）；清空回调 `onCleared`（`:206`）。

### 7.3 时间轴（`apps/viewer/src/replay/timeline.ts`，330 行）
...
### 7.4 录像信息条（`apps/viewer/src/ui/replaymeta.ts`，107 行）
...
### 7.5 遥测 HUD（`apps/viewer/src/ui/telemetry.ts`，93 行）
...
### 7.6 装配与对外 API（`apps/viewer/src/app.ts`，504 行）
