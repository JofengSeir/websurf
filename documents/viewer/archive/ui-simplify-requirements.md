...
> ⚠️ **本文档已被取代（2026-09-06）**：用户裁决转向「核心化简化」——地图游览 + 录像播放两核心，
> 量测/位姿 API/交互式标定/朝向诊断/规则表单编辑器等按**物理删除**处理，本文"不砍任何功能"
> 底线不再适用。现行方案见 `docs/core-simplify-plan.md`；本文仅作历史记录与 S4/S5 等已落地设计的参考。
