...
**补充对照（phys-rate-parity-v2.mjs 场景 A，2026-08-20）**：干净入坡（高空平飞自然落坡）——
- H=8/vz=500（纯坡面接触，无平台参与）：64Hz/144Hz 首碰 t=2.0781/2.0764、终速
  2423.6/2424.2 → **收敛**（真实坡面接触速率一致，幻影停驻已消失）；
- H=5/vz=500：144Hz 在 t=0.0833（z=15.14）被平台前缘掠触压至 y=0.03 减速、64Hz 直飞
  分叉——边缘掠触固有速率依赖；
- H=3/vz=500：盒落平台顶滑行（同 §10 第 1 条机制）。
此对照再次印证：分叉全部来自地面/边缘交互，门修复目标（幻影）两速率均达成。

**探针双速率**（phys-gate-probe2.mjs，2026-08-20）：64Hz 与 144Hz 均飞过坡，
final v=(0,-800,300) 两线一致；阻断 tick raw trace fraction=1.0 两线相同；门否决计数
16（64Hz）/54（144Hz，步数多）。**门在高步率同样生效。**
