...
> 协议细节见 [./implementation/shared-layout.md](./implementation/shared-layout.md) 与 [ts-shared.md](../ts-shared.md)。
...
- 要理解**为什么 harness 存在**（tick 无关的物理事实、双模设计动因）→ 工程根 [implementation/conclusion.md](./implementation/conclusion.md)。
...
- 要对照**仓库全局**（workspace、共享层引用矩阵、其他工程）→ [architecture.md](../architecture.md) 与 [ts-shared.md](../ts-shared.md)。
