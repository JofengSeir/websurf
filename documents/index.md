...
## 2. 文档树（documents/ 下全量 29 篇，含本文 index.md 自身；test/dual-mode-harness/docs/ 5 篇验证工程文档另列；另 archive/ 24 篇历史冻结）

### 根 documents/（5 篇：总架构 + 共享层四篇）
...
### documents/phys-plan-discuss/（4 篇：tick 模式方案讨论交付件）

| 文档 | 维度 |
|---|---|
| [t2-bench-brief.md](./phys-plan-discuss/t2-bench-brief.md) | 模拟台设计规格（场景基元库 / 三类旋转测试 / 忠实呈现口径 / 高刷流畅度指标） |
| [t5-user-test-guide-consumer-engineer.md](./phys-plan-discuss/t5-user-test-guide-consumer-engineer.md) | 真机手测指引交付件 |
| [t8-user-test-guide.md](./phys-plan-discuss/t8-user-test-guide.md) | tick 模式真机 A/B 手测指引（t8 交付物骨架） |
| [tick-mode-handover.md](./phys-plan-discuss/tick-mode-handover.md) | tick 权威模式交接参考（契约锚点 / 复跑命令 / 残留清单） |

### documents/discussion/（1 篇：跨团队讨论）

| 文档 | 维度 |
|---|---|
| [r3-memo-phys-researcher.md](./discussion/r3-memo-phys-researcher.md) | phys-researcher 立场备忘（r3） |
...
> 工程根说明文档（操作/部署）：[game/README.md](../game/README.md) · [viewer/README.md](../viewer/README.md) · [test/dual-mode-harness/README.md](../test/dual-mode-harness/README.md)（验证工程）。debug 工程无根 README，其操作说明见 [documents/debug/overview.md](./debug/overview.md)。
...
现行位置（上句为 2026-09 时点记录，其中 `docs/` 即现行 `documents/`）：`documents/archive/` · `documents/debug/archive/` · `documents/game/archive/` · `test/dual-mode-harness/docs/archive/` · `documents/viewer/archive/`。
