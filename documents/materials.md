...
> ③ 三条消费链：Rust 侧 GLB 构建期回退、JS 侧运行期画质切换、缺失纹理比对（本文承接三处已移出版本库的旧档 `documents/archive/materials.md`、`documents/debug/archive/materials.md`、`documents/game/archive/materials.md`，收敛为根单篇）。
...
| 共享数据 | `src/materials/textures.mtz`（5,942,995 B） | 与 `apps/debug/web/textures.mtz`、`apps/game/web/textures.mtz` 三处副本逐字节等大（`ls` 实测） | `ls -l` 实测 |
...
| ③ 缺失纹理比对（debug 独有） | 地图加载完成 | `showMissingTextures`（`apps/debug/src/app.ts:410-451`，由 `onSceneReadyUi` `:349` 触发）+ `loadDefaultTexturePack`（`default-pack.ts:18-31`） | 弹窗列出「连默认包都没有」的材质；回退本身已在链①自动应用（`app.ts:394-397` 注释） |

### 1.4 被引用关系（哪些工程拿到材质能力）

| 工程 | mosaic/默认纹理包能力 | 证据 |
|---|---|---|
| debug | 全套：`mosaic_encode/mosaic_decode/decompress_mtz` + `with_defaults/manifest/missing_textures` | `apps/debug/crates/wasm/src/lib.rs:3189-3204`（三全局函数）、`:494`（with_defaults）、`:850`（manifest）、`:874`（missing_textures） |
| game | 同 debug 全套（同源逐字复制） | `apps/game/crates/wasm/src/lib.rs:177-192`、`:435`、`:917`、`:942` |
| viewer | **不导出**——头注明示「brush/模型碰撞/teleport/PVS/mosaic/默认纹理包，均不导出」 | `apps/viewer/crates/wasm/src/lib.rs:9` |
...
bundle 产物携带 `mosaicManifest?/missingTextures?`（`:61-64`）→ 主线程 `handleLoadBsp` 组装 `SceneDataMessage` 交给渲染器：debug `app.ts:1305`、game `app.ts:462`；类型定义 `apps/debug/src/worker/worker-types.ts:298`。
...
- `textures.mtz` 源文件 `src/materials/textures.mtz`（5,942,995 B）；`apps/debug/scripts/build-dist.mjs`：single 模式读入并 base64 注入 `globalThis.__VBSP_TEXTURES_MTZ_B64__`（`:55`、`:115`）、multi 模式复制到 `dist/textures.mtz`（`:14`、`:185`）；
- 主线程 wasm 出口：`apps/debug/src/main-wasm.ts:10` 导入 `mosaic_decode/decompress_mtz`、`:45` 再导出（game 直接从 `../pkg/websurf_wasm.js` 导入，`apps/game/src/app.ts:14`）；
...
| 维度 | debug | game | 证据 |
|---|---|---|---|
| 链①构建期回退 | ✅（decompressMtz 注入） | ✅（同） | `apps/debug/src/app.ts:1295`、`apps/game/src/app.ts:451` |
| 链②画质切换 | ✅ mini/original | ✅ 同构（含 manifest 解析） | `apps/debug/src/renderer/renderer-main.ts:674-736`、`apps/game/src/renderer/renderer-main.ts:343-375` |
| `collectMissingTextures` | **true** | **不传**（无此能力消费） | `apps/debug/src/app.ts:1294`、`apps/game/src/app.ts:406-409` |
| 链③缺失比对弹窗 | ✅（default-pack.ts + showMissingTextures） | ❌ 无 default-pack 文件、无弹窗 | `apps/debug/src/default-pack.ts`（apps/game/src 无同名文件，grep 实证） |
| wasm 就绪门 | `ensureMainWasm`（main-wasm.ts 封装） | `app.ts:63/:138` promise（防 decompress_mtz 未就绪竞态） | `apps/game/src/app.ts:63,138` |
| Worker 侧 mtz | 协议兼容残留（见 §3.3） | 无 | `apps/debug/src/worker/main.ts:110` |

共同点：链①②两端行为一致（同一 world-builder、同一 applyTextureQuality 逻辑）；差异全部在链③与加载 UI（debug 缺失弹窗 vs game 进度覆盖层，见 `documents/debug/differences.md` §3.7）。

### 4.2 排除面（viewer / harness）

两端 cdylib 均不导出 mosaic/默认纹理包（§1.4 表）：viewer 以 GLB 内嵌材质为终态（构建期已烧入，无运行期切换需求）；harness 为时序验证最小导出集。因此 `textures.mtz` 只需 apps/debug/web、apps/game/web 两份部署副本 + src 源副本（三处逐字节相等，`ls -l` 实测）。
...
- debug 工程消费细节（applyTextureQuality 实现/近 missing 弹窗 UI/状态行）→ [./debug/implementation/rendering.md](./debug/implementation/rendering.md) 纹理画质切换节、[./debug/implementation/loading-pipeline.md](./debug/implementation/loading-pipeline.md) 纹理回退链节；
- game 工程消费 → [./game/implementation/gameplay.md](./game/implementation/gameplay.md)。

### 4.4 旧档承接

本篇收敛三处归档旧档（只读背景，内容未直接搬运、均经当前代码重验）：`documents/archive/materials.md`、`documents/debug/archive/materials.md`、`documents/game/archive/materials.md`（t1 归档时 `git mv` 纯重命名；这三个归档目录现已随工作区精简移出版本库，可在 git 历史中追溯）。
