...
| 工程 | 使用面 | 证据 |
|---|---|---|
| debug | 7 模块（auth×3、phys×3、input×1；**不含** compute-mode / tick-authority / decoupled-loop——worker 侧未注入任何模式钩子，缺省即纯耦合线），相对路径 `../../src/ts-shared/...` | `apps/debug/src/app.ts:26-31`、`apps/debug/src/worker/main.ts:27-30`、`apps/debug/src/renderer/renderer-main.ts:18-19`、`apps/debug/src/input/keyboard.ts:18` |
| game | **同为 7 模块同集**（`c4824e9` 回退后不再 import `decoupled/decoupled-loop.ts`；`apps/game/src/worker/main.ts:21-24` 仅 auth×3 + params） | `apps/game/src/config.ts:5`（buildPhysicsParams）、`apps/game/src/worker/main.ts:21-24`、`apps/game/src/app.ts:20-22`、`apps/game/src/renderer/renderer-main.ts:20-21`、`apps/game/src/input/keyboard.ts:11` |
