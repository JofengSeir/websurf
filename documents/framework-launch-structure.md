...
- 判据：`git grep -nE "\*\*\* ERROR|\[错误\]|\[提示\]|\[warn\]" -- apps src` 空输出（**目标：全仓 0 命中**；批 3 `32c2ddb`/`2135056` 后实测**已 0 命中**——判据保留为防回归；改造前为 `apps/viewer/scripts/build-dist.mjs` 7 处 + `src/serve.py` 2 处，处置见 §2.7 与 §10.1 第 10 条；`apps/*/play.cmd` 零命中）。
