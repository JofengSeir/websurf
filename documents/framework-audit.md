...
| viewer `play.cmd` | `exit /b 0`（复用已运行实例时，`:66`） | 5 处失败分支 `pause` + `exit /b 1` | `:12-17`、`:25-30`、`:35-40`、`:46-51`、`:55-60` |
| debug `start-dev.cmd` | `exit /b 0`（端口复用分支 `:68`；`cmd /k` 已由批 3 `32c2ddb` 移除） | 失败分支 `pause` | `:12-17`、`:29-34`、`:37-42`、`:48-53`、`:56-61` |
| debug `build-dist.cmd` | `exit /b 0` | 8 个失败分支 `pause` | `:12-15`、`:26-31`、`:37-42`、`:51-56`、`:59-64`、`:70-75`、`:79-84`、`:90-95` |
| game `build-dist.cmd` | `exit /b 0` | 7 个失败分支 `pause` | `apps/game/build-dist.cmd:11-15`、`:26-31`、`:37-42`、`:48-53`、`:59-64`、`:68-73`、`:79-84` |
| viewer `build-dist.cmd` | `exit /b 0` | 7 个失败分支 `pause` | `apps/viewer/build-dist.cmd:9-13`、`:24-29`、`:35-40`、`:46-51`、`:57-62`、`:66-71`、`:75-80` |

**退出码传递形态不一致（实测）**：游戏侧与 viewer 侧的 dist 步骤都用 `call node "%~dp0scripts\build-dist.mjs"`（直接调 node），viewer 还在注释里写明理由——「Direct node call（same as game\build-dist.cmd）：bypasses npm run so the exit code flows straight through」（`apps/viewer/build-dist.cmd:84-88`）；而 debug 侧第 3 步是 `call npm run build:dist`（`apps/debug/build-dist.cmd:63-64`）。同一仓库里「`npm run` 会吞退出码」这条经验只在 viewer 侧被写进注释并绕开，debug 未跟进。——**批 3 `32c2ddb` 已统一**：debug 的 `[5/5]`（`:86`）也改为 `call node "%~dp0scripts\build-dist.mjs"`（`:89`），viewer 对应 `:73`/`:74`，三工程一致。
...
| 失败前缀 | `[ERROR] …` | `[ERROR] …`（`*** ERROR: … ***` 已由批 3 `32c2ddb` 移除；`apps/game/build-dist.cmd:80`、`apps/viewer/build-dist.cmd:76` 两锚点现分别指向各文件的 `[ERROR] dist build failed.` 行） | `[ERROR] …` / `[HINT] …`（批 3 `32c2ddb` 后已统一；`apps/viewer/scripts/build-dist.mjs:77-78` 现为内嵌 `serve.py` 模板的 `[ERROR]`/`[HINT]` 行，即 `dist/serve.py` 的端口占用提示） |
...
| I-09 | 失败前缀三套：`[ERROR]` / `*** ERROR: ***` / `[错误]`——**更正**：本条初稿写「viewer 中英文混排」，实测 `apps/viewer/play.cmd` 为**纯 ASCII**（bytes >127 计数 = 0），中英混排实际出现在 `apps/viewer/scripts/build-dist.mjs` 与 `src/serve.py`，故问题性质是「**中文标记散落在构建脚本与共享脚本中**」，不是「viewer 入口中英混排」 | `apps/debug/play.cmd:26`（`[ERROR] npm install failed.`）、`apps/game/build-dist.cmd:80`（批 3 `32c2ddb` 前为 `*** ERROR: ***`，现为 `[ERROR] dist build failed.`）、`apps/viewer/build-dist.cmd:76`（同上）；中文标记批 3 前实测 **9 处**：`apps/viewer/scripts/build-dist.mjs` **7 处**（:77、:78、:161、:163、:164、:168、:169）+ `src/serve.py` **2 处**（:52、:53）——**批 3 `32c2ddb` 后全部改为 `[ERROR]`/`[HINT]`，实测 0 处**。**更正**：初稿把 `apps/viewer/build-dist.cmd:70` 列为 `[错误]` 出处——实测该文件 **0 处** `[错误]`/`[提示]` | 是否统一前缀与语言 |
...
| I-12 | Node 依赖引导三套：`ensure-node-deps.cmd`×2（字节全等）/ 内联 `if exist node_modules\esbuild` / 检测 `node_modules\.bin\tsc` | `apps/debug/build-dist.cmd:54`、`apps/viewer/build-dist.cmd:60`、`apps/debug/scripts/ensure-node-deps.cmd:13`、harness `play.cmd` 用 `node_modules\.bin\tsc` | 是否上提为 `src/scripts/ensure-node-deps.cmd`（**批 2 已执行**：两份工程内副本已上提共享单份 `src/scripts/ensure-node-deps.cmd`，viewer 两处内联探针亦改为同一调用，共 9 个入口调用点） |
...
| R-19 | 纯自举/环境准备类 shell 脚本（不含工程特有产物名的）必须在 `src/scripts/` 单份提供，工程内不得存副本 | `git ls-files 'apps/*/scripts/*.cmd'` 中不得出现 `ensure-node-deps.cmd`、`cargo-env.cmd` 之类通用名（当前两份 `ensure-node-deps.cmd` 违反 → I-15） |（**批 2 `fc3de84` 已执行**：`ensure-node-deps.cmd` 的两份工程内副本已上提为 `src/scripts/ensure-node-deps.cmd` 单份，`git ls-files -- 'apps/*/scripts/ensure-node-deps.cmd'` 实测空；`cargo-env.cmd` 本就在 `src/scripts/`） **【落地状态（收口 `t9` 追加，2026-09-12）】**：审计的 `I-01..I-22` 与 `R-01..R-21` 在批 1–4 后**部分已完成、部分仍待执行**，两者不得混淆——**已执行**：`I-02`/`I-03`/`I-22`（批 1）、`I-10`/`I-12`/`I-15`（批 2 `fc3de84`）、`I-07`/`I-08`/`I-09`/`I-11`/`I-13`/`I-14`/`I-16`/`I-21`（批 3 `32c2ddb`+`2135056`）、`R-17`/`R-18`/`R-19`/`R-20`/`R-21` 的落地面（批 2–4）；**仍待执行**：`test:smoke` 的 CI 化与改名（`I-02` 的 viewer 半边）、`test/dual-mode-harness` 另案（`I-10` 的 harness 半边、`I-12` 的 harness 半边）、`architecture.md` 之外的跨工程文档端口引用收尾。逐条处置与去向见 [rollout-status.md](rollout-status.md) §4–§5。
