...
- **全仓门禁（t3 复验通过后复跑）**：等价口径 doc-drift 输出 `50 篇 md ｜ 行数声明 154（漂移 0）｜锚点 1971（越界 0）｜路径失效 18（C 类告警）｜歧义未判 441`，**退出码 0**（数字随第二批 harness 并入与 t3 复核更正推进；A/B 两类仍为 0）；相对链接 100% 可达（本章末段口径）。
...
| `phys/authority-calibrator.ts` | 677 | 权威校准 |
| `phys/params.ts` | 62 | 物理参数映射 |
| `phys/world-builder.ts` | 249 | world bundle 组装 |
