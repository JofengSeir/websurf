...
- **全仓门禁（t3 更正闭环后复跑）**：等价口径 doc-drift 输出 `50 篇 md ｜ 行数声明 154（漂移 0）｜锚点 1966（越界 0）｜路径失效 18（C 类告警）｜歧义未判 440`，**退出码 0**（数字随第二批 harness 并入与 t3 复核更正推进；A/B 两类仍为 0）；相对链接 100% 可达（本章末段口径）。
