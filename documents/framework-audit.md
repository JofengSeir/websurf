...
| I-10 | 「谁提供 WASM 契约检查」三样：debug 是「导出符号 vs TS import 动态比对」（127 行），game 是「硬编码 16 + 17 个 API 名字」（80 行），viewer/harness 缺失或不同 | `apps/debug/scripts/check-wasm-api.mjs:109-126` 输出 `WASM 导出符号 (12)`；`apps/game/scripts/check-wasm-api.mjs:25-64` 的 `EXPORT_API`/`PHYS_API` 两数组 | 是统一为一个共享脚本，还是承认「导出集不同 → 检查项天然不同」 |
