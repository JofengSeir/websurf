...
| 体检项 | 命令 | 结果 |
|---|---|---|
| 文档漂移（A 行数声明 / B 锚点越界） | `node src/scripts/check-doc-drift.mjs`（成员沙箱内必然 `spawnSync git EPERM`，属环境限制；原命令实测 **exit 1**） | 等价口径实测（**t3 更正闭环后复跑**）：**50 篇 md ｜ 行数声明 154（漂移 0）｜锚点 1966（越界 0）｜路径失效 18 ｜歧义未判 440**，退出码 **0**。演进链（均实测）：批 4 收尾 `8879c15` 147 声明 / 1818 锚点；t1 `6695447` 后 149 / 1888；`5406e15` 后仍 149 / 1888；`1fe641e`/`c83b4c0` 后 150 / 1895；收口追加修订（`I-05`/`I-22` 与 §0.2/§7.4 口径、R-14）后 152 / 1917；R-3 归一 `df3a2d2` 后 152 / 1920；第二批（harness 并入 + rollout-plan 补注）后 152 / 1962；R-2 例外（两脚本保持独立实现，`bbb2b9c` + `c49e537`）后 154 / 1957；**t3 复核更正（F1 锚点、F2 端口、F3 判据口径、F4/F5 台账与本表新增第 7/8 行）后 154 / 1966** |
...
| 8 | **t3 复核后的更正（本节）**：`architecture.md:94` 锚点 `:13-14` → `:10-11`（**内容错位**，doc-drift 只查越界不报此；批前该文档用的正是 `:10-11`，harness 自己的 `docs/overview.md:101` 也一直是 `:10-11`）；`scripts/build-dist.mjs:70` 提示语 `8080` → `8110`；`R-12` 的「全仓 `IMPORTANT` 0 命中」判据改为限定「`.cmd` 输出标记」；`framework-audit.md:103` 门禁数字改为终态 `154/1966/440`；`R-2` 的「遗留」句改为已闭合 | 锚点由**实读行内容**核验（`:10-11` =「与 dev 产物同构…无 single 内嵌模式」）；`git grep -n "8080" -- test/` **零命中**；`git grep -n "echo.*\[IMPORTANT\]" -- "*.cmd"` **零命中**；doc-drift 等价口径 exit 0 |

## 7. 使用约束

- **本文档不改变任何规范条文的效力**：与三份规范冲突时以规范为准；与代码冲突时以代码为准并回改本文档。
- 本文档的**行数/计数类数字**（50 篇 / 154 / 1966 / 18 等）是实测快照；新增或修改文档后必须重跑 §3 的命令并同步更新，否则会被 `check-doc-drift.mjs` 记为漂移。
