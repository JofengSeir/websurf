...
| 批 5 | harness 并入规范（本团队第二批）：`R-2`/`R-10`/`R-11`/`R-12`——端口 `8110` + 引导/环境类共享脚本接入 + `check-wasm-api`/`build-dist` **保持自有独立实现** + `[IMPORTANT]` 归一 + `.cmd` 模板对齐 | 已落地 | `790e102`（代码 4 文件 + 本文件与三份规范、`architecture.md`、harness 文档、`CHANGELOG.md` 同步） |
...
| 体检项 | 命令 | 结果 |
|---|---|---|
| 文档漂移（A 行数声明 / B 锚点越界） | `node src/scripts/check-doc-drift.mjs`（成员沙箱内必然 `spawnSync git EPERM`，属环境限制；原命令实测 **exit 1**） | 等价口径实测（**t3 更正闭环后复跑**）：**50 篇 md ｜ 行数声明 154（漂移 0）｜锚点 1971（越界 0）｜路径失效 18 ｜歧义未判 441**，退出码 **0**。演进链（均实测）：批 4 收尾 `8879c15` 147 声明 / 1818 锚点；t1 `6695447` 后 149 / 1888；`5406e15` 后仍 149 / 1888；`1fe641e`/`c83b4c0` 后 150 / 1895；收口追加修订（`I-05`/`I-22` 与 §0.2/§7.4 口径、R-14）后 152 / 1917；R-3 归一 `df3a2d2` 后 152 / 1920；第二批（harness 并入 + rollout-plan 补注）后 152 / 1962；R-2 例外（两脚本保持独立实现，`bbb2b9c` + `c49e537`）后 154 / 1957；**t3 复核更正（F1 锚点、F2 端口、F3 判据口径、F4/F5 台账与本表新增第 7/8 行）后 154 / 1971** |
...
| 2 | `CHANGELOG.md:92`（第二批在 `:84` 之后插入 2 行，原 `:86`；2026-09-13 按**内容**重定位） | 同上 | 历史叙述（批 1 `I-22` 修复记录） | 保持，属批 1 事实记录；锚点按**内容**重定位到 `I-22` 子条目行 |
| 3 | `framework-audit.md:550`（§6.1 `I-12` 证据行；第二批按内容重定位，原写 `:548` 实为 `I-10` 行） | `apps/debug/scripts/ensure-node-deps.cmd` | **当前事实**已被 D-01 改变 | 保持原文；该行的现行处置结论见同文件 §6.4 的 `I-12` 行（批 2 `fc3de84` 上提共享单份）。经 t1 重新编号后，原记 `:434` 已失效（该行现为 `input/input-layer.ts` 行），批 2 上提补注位于 `:145` |
| 4 | `framework-audit.md:538`（§6.1 `I-22` 证据行；§6.4 状态行 `:592`）——第二批按内容重定位，原写 `:536`（实为 `I-05` 行）与 `:590` | `apps/debug/scripts/install-wasm-bindgen.cmd:19` | 历史叙述（批 1 缺陷描述） | 保持，属审计快照。原记 `:521` 已失效（该行现为**空行**）——经 t1 重新编号后 `I-22` 证据行迁至 `:536` |
...
| 16–18 | `rollout-status.md:50/:53/:56` | `apps/debug/scripts/install-wasm-bindgen.cmd`、`apps/*/src/world/pvs-manager.ts` | **引用原文**（§4 表自身的「旧引用」列） | 保持：这三处是本文档表格对旧路径的显式引用，用途即登记，不构成路径依赖。**本次实测重定位**：收口轮在第 3、7 条改写中新增行号引用使之由 `:44/:47/:50` 变为 `:49/:52/:55`；**第二批**又在 §1 追加「批 5」行（+1 行），实测现为 `:50/:53/:56`（content 口径：这三行分别是本表第 1/2/3 条的「旧引用」列） |
...
| R-2 | `test/dual-mode-harness/` 改造（**政策变更：并入规范**） | **已执行（第二批）** | [framework-decoupling.md](framework-decoupling.md) 的 `D-22` 原判「保留（本轮）」，本批改为**并入**并在该文件同步（`D-22` 裁决行、`§6.1` `T-03`/`T-04` 消费方行、`§6.3` harness 两行）。范围与证据：① 端口 `8080` → `8110`（`play.cmd:7`、`package.json:13`，见 R-11）；② 共享工具接入（**仅引导/环境类**）——`play.cmd:19` 调 `cargo-env.cmd`、`:23` 调 `ensure-node-deps.cmd nopause`、`:36` 调 `install-wasm-bindgen.cmd nopause`，全部 `%~dp0` 相对路径逐条 `Test-Path` 实测 **29/29 可达、0 不可达**（判据脚本 `.tmp/harness-converge/depth-check.mjs`，范围 = 三工程 9 入口 + harness 1 入口）；③ `check:api`/`build:dist` 的裁定见 R-10（**保持独立，不收敛**）；④ `[IMPORTANT]` 见 R-12；⑤ 输出模板对齐见 §6 第二批表第 1 行。**验证**：`npm run typecheck` / `build:ts` / `test:three-mode`（14 passed, 0 failed）exit 0；`build:dist` 属沙箱限制（esbuild JS API `spawn EPERM`）；`play.cmd` 探针实跑覆盖 happy path + `[SKIP]` + 6 个失败块（真实 HTTP 服务在 `8110` 启动、`GET /index.html` 200、PID 5344 终止后端口释放）。**遗留（已闭合）**：`rollout-plan.md` 的 G-3 裁定与 §10 第 3 条原先按「另案」表述，**第二批已在该文件就地补注**（S-2 行与 §10 第 3 条），故本行不再遗留。**端口补充（t3 复核后修）**：除 `play.cmd:7` 与 `package.json:13` 外，`scripts/build-dist.mjs:70` 的提示语原亦写 `serve.py 8080`——该文件在第二批曾随「回退为自带独立实现」被**一并回退**而重新带回旧端口，已单独改 `8110`；现 `git grep -n "8080" -- test/` **零命中** |
| R-3 | LF-only 行尾与 [AGENTS.md](../AGENTS.md)「行尾统一 CRLF」的矛盾 | **已闭环（本批 t2，纯字节级、内容零变化）** | **本次收口实测（宽口径，可复现）**：受检文本文件 **313**，其中 **LF-only 72 / CRLF 240 / 混合 0 / 无行尾 1**（`md/ts/mjs/js/json/cmd/rs/py/yml/yaml/toml/html/css`，取自 `git ls-files --cached --others --exclude-standard` 清单中磁盘存在的文件）。**与旧值的差额已归因**：原登记记「受检 308 / LF-only 71」，而 `8879c15` 与当前工作区的文本文件集**比对后无增无减**（均 313）——故差额**不是**收口新增文件所致，而是两次测量的**范围/口径差异**（无法从现行文档复原其精确口径）；本条以**本次可复现口径**为准。**处置方式**沿用 [AGENTS.md](../AGENTS.md) §7.1 第 8 项先例：逐文件验证「去掉 CR 后与 HEAD 字节全等」。**闭环实测（t2，纯字节级）**：全量 `git ls-files` 清单 **331** 个已跟踪文件中，按「有裸 LF 且无孤立/多 CR、无 NUL、无 BOM」过滤得**可归一文本文件 78 个**（换行符计数口径 27,088 行 / 1,011,565 B → 1,038,653 B，**增量 +27,088 B 恰等于裸 LF 数**）——其中白名单扩展名内 **72** 个、白名单外 **6** 个（`Cargo.lock` ×4、`apps/viewer/.gitignore`、`apps/viewer/test/node-shims.d.ts`），**上方登记值 72 与本清单 78 的差额即由这 6 个解释**；另 3 个 `.mtz` 纹理包（`i/-text`，含 NUL 与孤立 CR）与单行夹具 `apps/debug/fixtures/path/tick-on-render-prefix.json`（784,134 B、无行尾）**刻意排除**。逐文件判据：① 去 CR 后与归一前**字节快照**全等（78/78）；② 行数逐文件守恒（`27088 → 27088`）；③ 结果全 CRLF（`bareLF=0`、`loneCR=0`）且无行尾空白；④ `git hash-object` 与 index/HEAD **逐文件同 blob**（78/78 → 内容零变化证据）；⑤ 夹具与三份 `.mtz` 的 sha256 **归一前后不变**。复现命令与判据全文见 [CHANGELOG.md](../CHANGELOG.md) 0.2.0「修复」首条 |
| R-4 | `documents/architecture.md:207` 的历史引述（体检 C 项告警） | 已闭环 | 该行自述为刻意保留的历史记载，保持不动；C 项按「告警」口径对待 |
| R-5 | `documents/architecture.md:105` 的「`check-wasm-api.mjs` 存在于 debug / game / harness **三处**、viewer 无此脚本」断言 | **本次已修** | 该断言在批 2 后失真（viewer 已补薄配置、debug/game 已变薄配置）。已按实际改写为「**四工程各一份薄配置 + 共享引擎** `src/scripts/lib/wasm-api-contract.mjs`」（**第二批更正**：harness 那份为**自有独立实现**，该行随之改为「三工程薄配置 + 共享引擎；harness 自带独立实现」），并补「勘误」段与 debug/game 契约面差异、harness `:26-39` 的现行锚点；该处 +4 行使 `architecture.md` 总行数 199 → 203，其后的 `:202` 锚点顺延为 `:207`（见 R-4） |
...
| R-10 | harness 内 outOfScope 副本（`check-wasm-api.mjs`、`build-dist.mjs`） | **已裁定（第二批）：保持独立，不收敛** | 两脚本原为 56 / 76 行独立实现；第二批**曾按派单收敛**为「薄配置 64 行 + 共享引擎」与「薄入口 80 行 + 共享内核」，**随后经用户裁定否决并回退**——验证工程的门禁/构建脚本必须与被验对象独立，否则共享层出缺陷时两边一起错、验证失效。**终态**：`test/dual-mode-harness/scripts/{check-wasm-api,build-dist}.mjs` 仍为自带独立实现（56 / 76 行），`git grep -n "src/scripts" -- test/dual-mode-harness/scripts/` **零命中**；`check-wasm-api.mjs` 与 `df3a2d2` 同 blob（`90b9e164…`），`build-dist.mjs` 相对 `df3a2d2` **仅 1 行端口修正**（`:70` 提示语 `8080`→`8110`，见 §6 第 8 条；`git diff --numstat df3a2d2` = `1 1`）。**副本清单（更正值）**：`git ls-files '*check-wasm-api.mjs'` **恰 4 份** = 三工程薄配置 + harness 独立实现；`git ls-files '*/scripts/build-dist.mjs'` **恰 4 份** = 三工程薄入口 + harness 独立实现。验证：`node scripts/check-wasm-api.mjs` **exit 0** |
| R-11 | 批 3 端口判据中 `test/dual-mode-harness` 仍为 8080 | **viewer 半边已执行（批 3 `32c2ddb`）**；harness 半边**留待执行**（随 R-2） | **viewer 半边**：`test:replay` 的输出已迁 `.tmp/`（`apps/viewer/package.json:10` 的 `outfile=.tmp/replay-selftest/replay-selftest.mjs`，出处同为 `32c2ddb`；`git blame -L 10,11` 两行均 `32c2ddbf`）——原登记写「批 4 `b5be059`」系归因错误，已在本表更正，同步更正 [framework-audit.md](framework-audit.md) §8.5 的 `R-11` 行。**harness 半边仍未落地**：规范 §2.3 要求 harness 用 8110（与 `dev` 共用），现状 `test/dual-mode-harness/play.cmd:6` 仍 `set PORT=8080`、其 `package.json` 的 `dev` 亦未改；因该目录不在任何实现批次的 inScope，当时随 R-2 另案处置。**harness 半边（第二批已执行）**：`test/dual-mode-harness/play.cmd:7` 现为 `set PORT=8110`、`package.json:13` 的 `dev` 同为 `python ../../src/serve.py 8110 .`（行号 6 → 7 系第二批补 `chcp` 头部行）；`git grep -nE "set PORT=[0-9]+" -- apps test/dual-mode-harness` 实测取值集合 = {8080, 8081, 8090, 8091, 8100, 8101, 8110}，与规范 §2.3 表逐条一致，`8080` 只余 debug dev（规范值）。**闭环** |
| R-12 | **`[IMPORTANT]` 标记在词表之外**（本团队新发现） | 已登记（随 R-2，harness out-of-scope） | 实测全仓 `[IMPORTANT]` 仅 **1 处**：`test/dual-mode-harness/play.cmd:65`（`echo  [IMPORTANT] This window is the server. Keep it open while playing;`）。规范 §2.5 的唯一词表只含 `[0/5]`…`[N/M]`/`[ERROR]`/`[HINT]` 等，`[IMPORTANT]` **不在词表内**——三工程 9 个入口已按逐字模板清零词表外标记（批 3 `32c2ddb` + `2135056`），harness 是**唯一残留**。**处置结论（第二批已执行）**：按词表归一为 `[INFO]`——新位置 `test/dual-mode-harness/play.cmd:76`，且改为 §2.5.2 成功横幅**之后**的独立行，使横幅四行与模板保持逐字一致（`[IMPORTANT]` 原文在该横幅内）。同步把 §2.5.1 判据①的 pathspec 扩到 `test/dual-mode-harness/*.cmd`、把 §2.5.1 的 `[wasm-bindgen]` 例外触发面补上 harness。**验证**：`node .tmp/harness-converge/vocab-scope.mjs` → 13 个 `.cmd` 词表外标记 **0**。**判据口径更正（t3 复核）**：初稿写「`git grep -n "IMPORTANT"` 全仓 **0 命中**」**不成立**——实测全仓 5 文件 10 行命中（`src/scripts/install-wasm-bindgen.cmd:48` 为 `REM` 注释、`CHANGELOG.md` 2 处与三份规范/台账各若干处均为**描述词表的文字**）。正确判据限定为「**`.cmd` 输出标记**中 `[IMPORTANT]` 为 0」，即 `git grep -n "echo.*\[IMPORTANT\]" -- "*.cmd"` 零命中（`test/dual-mode-harness/play.cmd` 已归一为 `[INFO]`）；仓库内出现「IMPORTANT」字样本身不构成缺陷 |
...
| R-14 | **`documents/viewer/README.md:45` 仍写「本地地图副本放仓库根 `maps/`」**（`I-05` 的 viewer 半边，收口时实测发现） | **已执行（收尾轮闭环）** | 实测 `documents/viewer/README.md:45` 原文 = 「更换地图」（拖拽 `.bsp` 仍全局可用）。本地地图副本放仓库根 `maps/`（gitignored）。而根 `README.md:43` 已统一为 **`test/maps/`** 并声明「仓库根 `maps/` 已废弃」（出处 `a4ed66f`）。**处置结论**：该文件属 `apps/`，本不在 t3 inScope，故收口期间**只登记不修**，`framework-audit.md` §6.4 的 `I-05` 相应记为**「部分执行」**；**收尾轮**由 captain 直接改为 `test/maps/` 并补指根 README §4，`I-05` 的 viewer 半边随之**闭环**（该行 §6.4 状态后续可改记为「已执行」）。**去向**：已闭环，无后续动作 |

## 6. 改动记录（`t9` 历史段 + `t3` 收口段 + 第二批段）
...
| 8 | `architecture.md:105` | `check-wasm-api` 断言改写为「四工程薄配置 + 共享引擎」并补勘误段（**第二批更正**：harness 那份实为自有独立实现，该行已在第二批改为「三工程薄配置 + 共享引擎，harness 自带独立实现」） | +4 行 → 其后 `:202` 顺延 `:207`（已同步本文档） |
...
| 11 | `CHANGELOG.md:99`（2026-09-13 按**内容**重定位，原 `:93`） | 占位条目「批 2/3/4 尚未落地」替换为**批 2/批 3/批 4/`D-23` 搬迁/本次收口**五条实际条目（附提交号与验证方式） | 批 1 条目保持原样 |
...
| 5 | 本文档 §5 | R-1/R-11 改记已执行（`32c2ddb`）并保留成因说明；R-8 更新为「`640da1a` 已登记例外 + 判据白名单化」；**新增 R-12**（`[IMPORTANT]` 词表外，harness）、**新增 R-13**（`Q4` 登记，非缺陷） | `git blame -L 10,11 -- apps/viewer/package.json`；`git grep -n "IMPORTANT"` 实测 1 处（**收口当时值；第二批归一后此计数不再有效，现为 5 文件 10 行且均非 `.cmd` 输出标记，见 §5 的 `R-12` 判据口径更正**）；`src/scripts/lib/dist-pack.mjs:107` |
...
| 7 | 本文档 §5 | 新增 **R-14**（`documents/viewer/README.md:45` 旧路径，只登记不修） | 与 §6.4 的 `I-05`「部分执行」一致 |

> 下行为 captain 第二轮裁定（「已执行」证据形式放宽为**二选一**）后的补正。

| # | 文件 | 改动 | 判据 |
|---|---|---|---|
| 1 | `framework-audit.md` §8.5 `R-04` | 原依据「审计当时即合规；批 3 后 9 个入口 `.cmd` 仍…（§2.4 实测）」属**无 sha / 无条款号的模糊依据**，已改为二选一形式：补**提交号** `32c2ddb` + `2135056`，并给出**逐字节复核结论**（13 个被跟踪 `.cmd` 全部合规：BOM 0 / 孤立 CR 0 / 裸 LF 0 / 字节 >127 = 0），再引规范 `framework-launch-structure.md` §8.2（`:708`）与 §3.2（`:413`） | 复核脚本 `node .tmp/check-r04.mjs`；两个条款行经 node 按字节读确认标题逐字相符 |
| 2 | 同理复核其余 6 行 | `I-05`（已改部分执行）、`I-14`/`I-17`/`I-18`/`I-19`（原文即以「规范 §3.2/§8.2 + 具体豁免内容」为据）、`R-21`（规范 §8.2 例外表）**均已符合 (b) 形式**，故**未改** | 逐行实读；`§8.2`（`:708`）与 `§3.2`（`:413`）实测存在 |

> 下行为 captain 第三轮授权（`§6.1` 的 `I-04`/`I-22`/`I-21` 三行 + `§0.2` 末自述）的处置。

| # | 文件 | 改动 | 判据 |
|---|---|---|---|
| 1 | `framework-audit.md` §6.1 `I-04` 行（`:535`） | 追加**收口补注**：批 4 `b5be059` 已删失效 include，现值为 `["src","../../src/ts-shared/**/*.ts"]`；本行为审计当时快照，处置结论见 §6.4 的 `I-04` | `git show b5be059 -- apps/debug/tsconfig.json` 显示 `- [...,"web/vendor",...]` → `+ [...]`（该 diff 只此一处）；`apps/debug/tsconfig.json` 现文实测一致 |
| 2 | `framework-audit.md` §6.1 `I-22` 行（`:538`） | 追加**收口补注**：证据列为审计当时快照；脚本已由批 2 `fc3de84` 上提 `src/scripts/`，旧路径实测不存在，故「第 19 行」等行号不再指向现行文件；处置结论见 §6.4 的 `I-22` | 旧路径 `apps/debug/scripts/install-wasm-bindgen.cmd` 实测缺失；`src/scripts/install-wasm-bindgen.cmd` 存在（105 行） |
| 3 | `framework-audit.md` §6.1 `I-21` 行（`:539`） | 追加**收口补注**：批 3 `32c2ddb` 已按唯一源方案消除本缺口（`src/phys/{LICENSE,NOTICE}` 单份 + `copyLicensePair` 产物级拷贝），「只有 debug 拷贝、game 无对应代码」已不反映现状；处置结论见 §6.4 的 `I-21` | `src/scripts/lib/dist-pack.mjs:163` `export async function copyLicensePair(`；`apps/{debug,game}/scripts/build-dist.mjs` 各自 `:165`/`:162` 调用 |
| 4 | `framework-audit.md` §0.2 末自述 | 原句「已修 §6.1 的 `I-04`/`I-22`/`I-21`」改为**「三行的处置结论统一见 §6.4（本节只记错位与补注，不重复落地状态）」** —— 同一事实**只在一处维护** | 采纳 captain 的「指向单一处比两处都写更安全」；避免本会话反复出现的「同一句子两种写法」 |

> 下行为**第二批（harness 并入规范，`D-22` 政策变更：由「本轮保留」改为「部分并入」——引导脚本与端口/输出模板并入，门禁/构建脚本按裁定保持自带实现）**的改动。

| # | 文件 | 改动 | 判据 |
|---|---|---|---|
| 1 | `test/dual-mode-harness/play.cmd` | 端口 `set PORT=` `8080` → `8110`；补 §2.4.1 头部 5 行（`chcp 65001` + `title WebSurf-test - Play`）；`[1/3]` 步改调共享 `ensure-node-deps.cmd nopause`（删 `node_modules\.bin\tsc` 探针与内联 `npm install`）；`[2/3]` 的 WASM 子步骤新增共享 `install-wasm-bindgen.cmd nopause` + 两行 `[INFO]`；补 §2.3 端口占用检测与 §2.5.5 `[SKIP]` 复用块；`[IMPORTANT]` → `[INFO]`；六个失败块改「恰一条 `[HINT]`」两行制；分隔线统一 60 字符 | `node .tmp/harness-converge/vocab-scope.mjs`：13 个 `.cmd` 抽出标记 222 处／去重 20 = 词表 18 + 登记例外 2、**词表外 0**；`[ERROR]` 57 个 → 紧跟 `[HINT]` 57 个（违规 0）。探针实跑 8 条路径（happy path／`[SKIP]`／6 个失败块），其中真实 HTTP 服务在 `8110` 启动、`GET /index.html` → 200、终止后端口释放 |
| 2 | `test/dual-mode-harness/package.json:13` | `dev` 的 `serve.py` 端口 `8080` → `8110`（与 `play.cmd` 同端口属规范 §2.3 明文【豁免】） | `git grep -nE "serve\.py [0-9]+" -- test/dual-mode-harness` 实测 `8110`；`git grep -n "8080" -- test/dual-mode-harness` 零命中 |
| 3 | `test/dual-mode-harness/scripts/check-wasm-api.mjs` | **保持自带独立实现（56 行）**——原派单要求「收敛为薄配置 + 共享引擎（D-03/T-03）」经用户在执行中**否决**：验证工程的门禁脚本若依赖被验对象，共享层出缺陷时两边一起错、验证失效。接入 `check:api` 与端口/输出模板规范，但不引共享引擎 | `git grep -n "src/scripts" -- test/dual-mode-harness/scripts/` **零命中**；文件 import 仅 `node:fs`／`node:path`／`node:url`（实测）；`node scripts/check-wasm-api.mjs` **exit 0**（`✓ WASM 契约通过：PhysWorld 类 + 12 个方法全部导出。`）；`git diff df3a2d2 -- test/dual-mode-harness/scripts/check-wasm-api.mjs` 为空（与薄配置前逐字节相同） |
| 4 | `test/dual-mode-harness/scripts/build-dist.mjs` | **保持自带独立实现（76 行）**——同上否决「薄入口 + 共享内核（D-04/T-04）」，理由同第 3 条 | `git grep -n "src/scripts" -- test/dual-mode-harness/scripts/` **零命中**；import 仅 `esbuild`／`node:fs`／`node:path`／`node:url`（实测）；与 `df3a2d2` 比对：**除 `:70` 提示文案 `8080` → `8110` 一处外逐字节相同**（该处系 `t3` 的 F2 修正，故 `git diff df3a2d2 -- …build-dist.mjs` 为 1 insertion / 1 deletion、**不为空**）；`check-wasm-api.mjs` 则与 `df3a2d2` **完全相同**（blob `90b9e164…`） |
| 5 | `test/dual-mode-harness/src/renderer/tick-consumer.test.ts:20`、`src/panel/tick-telemetry-format.test.ts:6` | 注释内写死的 `--outfile=temp/…` 与 `node temp/…` → `.tmp/…`（规范 §3.5／R-11） | `git grep -n "temp/" -- test/dual-mode-harness` 实测 **0 命中** |
| 6 | 三份规范 + `architecture.md` + harness 文档 + `CHANGELOG.md` | 「harness 另案／未收敛」表述按实测同步：`D-22` 政策变更（**部分并入**：引导脚本与端口/输出模板并入；门禁/构建脚本按裁定保持独立）、`T-03`/`T-04` 消费方、`I-10`/`I-12` 的 harness 半边、`R-02`/`R-10` 状态、规范 §2.1/§2.3/§2.5.1/§2.7/§8.2、`architecture.md` 的契约校验段与 `build-dist.mjs` 锚点、harness `documents/dual-mode-harness/overview.md` 的行数声明与锚点。**更正**：初版曾把两脚本写作「四份薄配置 + 共享引擎」与「薄入口」，该表述已按第 3/4 条的裁定逐处改回「harness 自带实现」 | 每处 `文件:行号` 锚点按**行内容**核验（禁止行号换算）；`git grep -n "第二批收敛\|第二批已收敛\|第二批已并入\|已消费共享引擎" -- documents/ test/` 复核后**不得再有把 harness 两脚本说成薄形态的表述**；全仓 doc-drift 等价口径复跑见 §3 |
| 7 | **范围偏差（t3 复核后如实登记）**：`R-3` 归一实际覆盖了 `test/dual-mode-harness/**` 12 个文件，而 `t2` 派单的 outOfScope 写的是排除该目录 | 仅行尾 CRLF 归一（补 CR），**内容零变化**——判据：这些文件 `stripCR(工作区) == HEAD blob`（t3 全量 328/328 通过）、`git diff` 无内容差异、`git ls-files --eol` 全仓 `i/lf w/crlf` 327 + `i/none` 1 + `i/-text` 3、`w/lf` 残留 **0**。无损失、无需回退 | 取向说明：`AGENTS.md` §5.2 的「行尾统一 CRLF」是仓库级约定，验证工程无正当理由例外；且该目录随后由批 5 正常修改，故按「结果一致且无损」接受，仅登记范围偏差以备追溯 |
| 8 | 本节（`architecture.md:94`、`scripts/build-dist.mjs:70`、`R-12` 判据、`framework-audit.md:103`、`R-2` 遗留句） | **t3 复核后的更正**：`architecture.md:94` 锚点 `:13-14` → `:10-11`（**内容错位**，doc-drift 只查越界不报此；批前该文档用的正是 `:10-11`，harness 自己的 `documents/dual-mode-harness/overview.md:101` 也一直是 `:10-11`）；`scripts/build-dist.mjs:70` 提示语 `8080` → `8110`；`R-12` 的「全仓 `IMPORTANT` 0 命中」判据改为限定「`.cmd` 输出标记」；`framework-audit.md:103` 门禁数字改为终态 `154/1971/441`；`R-2` 的「遗留」句改为已闭合 | 锚点由**实读行内容**核验（`:10-11` =「与 dev 产物同构…无 single 内嵌模式」）；`git grep -n "8080" -- test/` **零命中**；`git grep -n "echo.*\[IMPORTANT\]" -- "*.cmd"` **零命中**；doc-drift 等价口径 exit 0 |

## 7. 使用约束

- **本文档不改变任何规范条文的效力**：与三份规范冲突时以规范为准；与代码冲突时以代码为准并回改本文档。
- 本文档的**行数/计数类数字**（50 篇 / 154 / 1971 / 18 等）是实测快照；新增或修改文档后必须重跑 §3 的命令并同步更新，否则会被 `check-doc-drift.mjs` 记为漂移。
