...
| 4 | `test/dual-mode-harness/scripts/build-dist.mjs` | **保持自带独立实现（76 行）**——同上否决「薄入口 + 共享内核（D-04/T-04）」，理由同第 3 条 | `git grep -n "src/scripts" -- test/dual-mode-harness/scripts/` **零命中**；import 仅 `esbuild`／`node:fs`／`node:path`／`node:url`（实测）；`git diff df3a2d2 -- test/dual-mode-harness/scripts/build-dist.mjs` 为空 |
