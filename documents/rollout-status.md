...
| 2 | `CHANGELOG.md:90`（第二批在 `:84` 之后插入 2 行，原 `:86`） | 同上 | 历史叙述（批 1 `I-22` 修复记录） | 保持，属批 1 事实记录；锚点按**内容**重定位（`:90` = `I-22` 子条目行） |
