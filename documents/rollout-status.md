...
| R-10 | harness 内 outOfScope 副本（`check-wasm-api.mjs`、`build-dist.mjs`） | **已执行（第二批）** | `check-wasm-api.mjs`：56 行**独立实现** → **薄配置 64 行 + 共享引擎**（`test/dual-mode-harness/scripts/check-wasm-api.mjs:16-19` 引 `src/scripts/lib/wasm-api-contract.mjs`，`:26-39` 保留本工程 12 API 清单）；`build-dist.mjs`：76 行**独立实现** → **薄入口 80 行 + 共享内核**（`test/dual-mode-harness/scripts/build-dist.mjs:23-28` 引 `src/scripts/lib/dist-pack.mjs`）。**收敛后副本清单**：`git ls-files '*check-wasm-api.mjs'` **恰 4 份**、`git ls-files '*/scripts/build-dist.mjs'` **恰 4 份**，均为「薄配置/薄入口 + 共享实现」——**未产生第 4 份独立实现**。验证：`node scripts/check-wasm-api.mjs` **exit 0**；`npm run build:dist` 的等价口径见 §6 第二批表第 4 行 |
...
> 下行为**第二批（harness 并入规范，`D-22` 政策变更：由「本轮保留」改为「并入」）**的改动。
...
| 3 | `test/dual-mode-harness/scripts/check-wasm-api.mjs` | 第 3 份独立实现（56 行）→ **薄配置**（64 行）+ 共享引擎 `src/scripts/lib/wasm-api-contract.mjs`（D-03/T-03） | `:16-19` 引共享引擎、`:26-39` 保留本工程 12 API 清单；`node scripts/check-wasm-api.mjs` **exit 0**（`✓ WASM 契约通过：PhysWorld 类 + 12 个方法全部导出。`） |
| 4 | `test/dual-mode-harness/scripts/build-dist.mjs` | 第 4 份独立实现（76 行）→ **薄入口**（80 行）+ 共享内核 `src/scripts/lib/dist-pack.mjs`（D-04/T-04） | `:23-28` 引内核；`npm run build:dist` 在沙箱内 `spawn EPERM`（esbuild JS API，环境限制，非缺陷）→ 等价口径（同一 esbuild 二进制 CLI + 内核 `cleanDist`/`cleanStale`/`printTree` 装配）实测产出 5 文件 `dist/`（app.js／worker-a.js／worker-b.js／wasm／index.html），`cleanStale` 按 KEEP 断言生效 |
...
| 6 | 三份规范 + `architecture.md` + harness 文档 + `CHANGELOG.md` | 「harness 另案／未收敛」表述按实测同步：`D-22` 政策变更、`T-03`/`T-04` 消费方、`I-10`/`I-12` 的 harness 半边、`R-02`/`R-10` 状态、规范 §2.1/§2.3/§2.5.1/§2.7/§8.2、`architecture.md` 的四份薄配置断言与 `build-dist.mjs` 锚点、harness `docs/overview.md` 的行数声明与锚点 | 每处 `文件:行号` 锚点按**行内容**核验（禁止行号换算）；全仓 doc-drift 等价口径复跑见 §3。本批按内容重定位的旧锚点：§4 第 2 条 `CHANGELOG.md:86` → `:90`、第 3 条 `framework-audit.md:548` → `:550`、第 4 条 `:536`/`:590` → `:538`/`:592`、第 16–18 条 `:49/:52/:55` → `:50/:53/:56`（每条均逐字节实读行内容确认） |
## 7. 使用约束
