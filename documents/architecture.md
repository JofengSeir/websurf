...
| harness | 多文件 dist（5 文件，无 single 内嵌；`build-dist.mjs` 为**本工程自带实现**，按 R-2 例外不消费共享内核 `dist-pack.mjs`） | 不部署 | 仅消息回退模式等价可用 | `test/dual-mode-harness/scripts/build-dist.mjs:13-14`、overview §5 |
