...
use websurf_wasm_core::model_integrator::map_coords;
use crate::shell_colliders as sc;
use websurf_wasm_core::vbsp;
use websurf_wasm_core::phyfile;
use websurf_wasm_core::pakfile_models::{self as pk};
...
fn probe_phy_stats() {
    ...
    let bsp = vbsp::Bsp::read(&data).expect("BSP 解析失败");
    let (models, _props, entry_names) = crate::collect_pakfile_models(&bsp).expect("collect");
    let index = pk::PakIndex::build(&entry_names);
    ...
    for m in &models {
        ...
        has_phy += 1;
        match phyfile::parse_phy(&phy_bytes) {
            Ok(solids) => {
                parsed_ok += 1;
...
fn probe_phy_mapping() {
    ...
    for m in &models {
        ...
        let Ok(solids) = phyfile::parse_phy(&phy_bytes) else {
            continue;
        };
...
fn probe_ramp() {
    ...
    for mesh in model.meshes() {
        ...
        for c in idx.chunks_exact(3) {
            ...
            sc::push_oriented_tri(&mut tris, [local[a], local[b], local[d]], hint);
        }
    ...
    // ---------------- 7. 实例数 ----------------
    let static_props: Vec<model_integrator::StaticProp> = bsp
        .static_props()
        .enumerate()
        .map(|(_i, p)| model_integrator::StaticProp {
        ...
        .collect();
    let no_ent: Vec<model_integrator::Entity> = Vec::new();
    let placements = model_integrator::resolve_placements(&mdl_entry, &no_ent, &static_props);
    ...
    let (corners, _) = sc::placed_obb(vmin, vmax, p0.translation, p0.rotation, p0.scale);
    ...
    for t in &tris {
        let face = sc::tri_to_face(*t);
        let Some((wv, wn)) = sc::transform_face(&face, p0.translation, p0.rotation, p0.scale) else {
            continue;
        };
        let Some(b) = sc::face_to_brush(&wv, wn, collider_thickness) else {
            continue;
        };
        raw_n += 1;
...
fn brush_solid_vol_estimate(b: &sc::BrushOut, m: u32, seed: u64) -> f32 {
