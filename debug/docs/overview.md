...
| `src/world/` | pvs-manager / spawn-loader（未接线/预留——出生点实际由共享 world-builder 解析）/ collider-adapter（可视化用）/ teleport-manager（可视化用）/ custom-teleports（自定义传送点持久化）/ types |
...
| 渲染/调试 | 场景构建、PVS/LOD、lightmap、雾、碰撞可视化（brush/trigger/phy/vis/**chamfer 切角平面**）、准星射线、近平面自适应 | `debug/docs/rendering.md` |
...
| 调试线框 | 碰撞可视化 5 开关 + 5 距离滑块（brush / trigger / phy / vis / **chamfer 切角平面黄色线框**） |
| 元数据 | 元数据 KV |
...
- **调试 API**：`parse_entities` / `list_pakfile` / `read_pakfile_*` / `parse_bsp` / `export_visleaf_pvs`——仅 debug 导出（WASM 导出层方法，非 window 全局）。
- **chamfer 切角平面可视化**：把 WASM 导出层为凸棱运行时生成的切角平面单独标黄显示（排查坡顶/尖角幻影碰撞；详见 `./rendering.md` §4）。
