...
## 1. 启动时序（main()，`debug/src/app.ts:198-293`）
...
Worker 侧接收 `wasm-init`：`initSync({module})` **必须同步初始化**（async `init()` 解构 `{module_or_path}`，传 `{module}` 会落到 `new URL(import.meta.url)` 错误路径——`src/ts-shared/auth/worker-dispatch.ts:50-60` 注释），成功后 `authLoop.start()` 一次（`worker-dispatch.ts:68-72`）。`init` 消息（含 SAB）到达后创建 `createWorkerSharedState` 并回 `ready`（`worker-dispatch.ts:79-85` + `debug/src/worker/main.ts:106-108`）。
...
### 3.1 渲染物理线（主线程 rAF，`debug/src/renderer/renderer-main.ts:430-516`）
...
- `fixedDt = 1/tickRate`：config `physics.tickRate` 变更时主线程 `sendConfig` → Worker `getConfigTickRate()`（`debug/src/worker/main.ts:98`）→ `setFixedDt(rate) + reset()`（`worker-dispatch.ts`；主线程侧 `app.ts` tickRate 滑块 → `physicsWorker.params.onTickRateChange` → `authLoop.setFixedDt`，`debug/src/worker/main.ts:89-92`）。
